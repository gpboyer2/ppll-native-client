...
const saveCredentials = async () => {
  ...
  const apiKey = inputs.apiKeyInput.value.trim()
  const apiSecret = inputs.apiSecretInput.value.trim()
  ...
  try {
    ...
    if (res && res.code === 0 && res.data) {
      ...
      if (apiKey && apiSecret) {
        const inputs = getCredentialsInputs()
        if (inputs) {
          inputs.apiKeyInput.value = apiKey
          inputs.apiSecretInput.value = apiSecret
          currentCredentials = { apiKey, apiSecret }
...
const testCredentials = async () => {
  ...
  if (!BinanceApi.validateCredentials(apiKey, apiSecret)) {
  ...
  try {
    ...
    if (response.code === 0 && response.data?.status === 'success') {
      ...
      showStatus(`连接测试失败: ${response.data?.message || response.msg}`, 'error')
    }
  } catch (error) {
...
const refreshAccountInfo = async () => {
  ...
  try {
    ...
    if (response.code === 0 && response.data?.status === 'success') {
      ...
      showStatus(`获取账户信息失败: ${response.data?.message || response.msg}`, 'error')
    }
  } catch (error) {
...
const getPositionConfigs = (): PositionConfig[] => {
  ...
  positionItems.forEach(item => {
    ...
    if (symbolInput && longInput && shortInput) {
      ...
      const longAmount = parseFloat(longInput.value) || 0
      const shortAmount = parseFloat(shortInput.value) || 0
      
      if (symbol && longAmount > 0 && shortAmount > 0) {
        configs.push({ symbol, longAmount, shortAmount })
      }
    }
  })
...
const buildPositions = async () => {
  ...
  try {
    ...
    if (response.code === 0 && response.data?.status === 'success') {
      ...
      showStatus(`建仓操作失败: ${response.data?.message || response.msg}`, 'error')
    }
  } catch (error) {
...
const closePositions = async () => {
  ...
  try {
    ...
    if (response.code === 0 && response.data?.status === 'success') {
      ...
      showStatus(`平仓操作失败: ${response.data?.message || response.msg}`, 'error')
    }
  } catch (error) {
