    ...
    <div className="info-card">
      ...
      <div className="info-card-body">
        <div className="info-row">
          ...
          <span className="info-value">{total_open_position_quantity.toFixed(DECIMAL_PLACES.QUANTITY)}</span>
        </div>
        <div className="info-row">
          <span className="info-label">持仓价值</span>
          <span className="info-value">{total_open_position_value.toFixed(DECIMAL_PLACES.PRICE)} USDT</span>
        </div>
        <div className="info-row">
          <span className="info-label">成本价格</span>
          <span className="info-value">{total_open_position_entry_price.toFixed(DECIMAL_PLACES.PRICE)}</span>
        </div>
        {current_price && (
          <div className="info-row">
        ...
        {current_price && total_open_position_entry_price && (
          <div className="info-row">
            ...
            <span className={`info-value ${price_change >= 0 ? 'positive' : 'negative'}`}>
              {price_change >= 0 ? '+' : ''}{price_change.toFixed(DECIMAL_PLACES.PERCENTAGE)}%
            </span>
          </div>
        )}
        {liquidation_price && (
