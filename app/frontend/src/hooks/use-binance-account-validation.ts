  ...
  const validate = useCallback(async (options: UseBinanceAccountValidationOptions) => {
    ...
    try {
      ...
      if (response.status === 'success' && response.datum) {
        ...
        setResult({ status: 'error', error: error_message, errorType: error_type, ipAddress: ip_address });
        return { status: 'error' as const, error: error_message, errorType: error_type, ipAddress: ip_address };
      }
    } catch (error: any) {
...
export type { AccountValidationData, ValidationResult, ErrorType, UseBinanceAccountValidationOptions };
