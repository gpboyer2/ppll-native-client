...
async function testModelAttributes() {
  try {
    ...
    console.log(`模型原始属性数量: ${Object.keys(model.rawAttributes).length}\n`);
    ...
    fields.forEach((attr, index) => {
      ...
      console.log(`  type: ${attr.type.toSql ? attr.type.toSql() : String(attr.type)}`);
    ...
    const modelFields = Object.values(model.rawAttributes).map(attr => attr.fieldName);

    const onlyInPragma = pragmaFields.filter(f => !modelFields.includes(f));
    const onlyInModel = modelFields.filter(f => !pragmaFields.includes(f));
