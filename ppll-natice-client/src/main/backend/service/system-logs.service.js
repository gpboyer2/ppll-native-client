...
function toViewObject(data) {
    ...
    const operation =
        extra.operation ||
        (data.http_method && data.api_endpoint
            ? `${data.http_method} ${data.api_endpoint}`
            : "未知操作");
...
async function create(req) {
    ...
    if (!api_endpoint)
        throw new ApiError(httpStatus.BAD_REQUEST, "缺少必要参数 api_endpoint");

    // 解析 UA 信息
    const { os: os_name, browser: browser_name } = parseUa(
        body.user_agent || req.headers["user-agent"],
    );

    // request/response 可能为对象或字符串，统一序列化为字符串存库
    const request_data =
        typeof body.request_data === "object"
            ? JSON.stringify(maskObject(body.request_data))
            : body.request_data || null;
    const response_data =
        typeof body.response_data === "object"
            ? JSON.stringify(maskObject(body.response_data))
            : body.response_data || null;
    const ip_address =
        body.ip || req.ip || req.headers["x-forwarded-for"] || null;
    const location = body.location || null;
    const user_agent = body.user_agent || req.headers["user-agent"] || null;
    const response_time =
        body.response_time || deriveDuration(body.extra_data) || null;
...
async function batchCreate(req, logs = []) {
    ...
    const rows = logs.map((l) => {
        ...
        return {
            ...
            request_data:
                typeof l.request_data === "object"
                    ? JSON.stringify(maskObject(l.request_data))
                    : l.request_data || null,
            response_data:
                typeof l.response_data === "object"
                    ? JSON.stringify(maskObject(l.response_data))
                    : l.response_data || null,
            ip_address: l.ip || ip,
            ...
            browser_name,
            response_time:
                l.response_time || deriveDuration(l.extra_data) || null,
            extra_data: maskObject(l.extra_data || null),
            created_at: l.request_time
                ? new Date(l.request_time)
