...
const createApiKey = async (params) => {
    ...
    console.log(
        `${API_KEY_LOG_PREFIX} CREATE_SUCCESS id=${createdKey.id} name=${name} api_key=${api_key.substring(0, 8)}...`,
    );
...
const deleteApiKeyById = async (api_key_id) => {
    ...
    console.log(
        `${API_KEY_LOG_PREFIX} DELETE_SUCCESS id=${api_key_id} name=${api_key.name} api_key=${api_key.api_key.substring(0, 8)}...`,
    );
...
const updateApiKeyById = async (api_key_id, updateBody) => {
    ...
    if (invalidFields.length > 0) {
        throw new ApiError(
            ...
            `禁止修改以下字段: ${invalidFields.join(", ")}`,
        );
    }

    const { name, api_key, api_secret, status, remark, vip_expire_at } =
        updateBody;
    const updateData = {};
    ...
    console.log(
        `${API_KEY_LOG_PREFIX} UPDATE_SUCCESS id=${api_key_id} fields=${Object.keys(updateData).join(",")}`,
    );
...
const getAllApiKeys = async (filter = {}, options = {}) => {
    ...
    console.log(
        `${API_KEY_LOG_PREFIX} QUERY filter=${JSON.stringify(filter)} result_count=${apiKeys.length} ids=${apiKeys.map((k) => k.id).join(",")}`,
    );
    return apiKeys;
};
