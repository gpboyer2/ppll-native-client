        ...
        await OperationLogModel.create({
            ...
            extra_data:
                errorCode || error_message
                    ? { error_code: errorCode, error_message: error_message }
                    : null,
...
async function logUserAction(
    ...
    try {
        ...
        const module_name =
            options.module ||
            (page ? String(page).replace(/^\//, "").split("/")[0] : null);
        ...
        const status =
            typeof options.status === "number"
                ? options.status === 1
                    ? 1
                    : 0
                : extraData && (extraData.error || extraData.error_message)
                  ? 0
                  : 1;
        const operation_time = options.operationTime
            ? new Date(options.operationTime)
            : new Date();
    ...
    try {
        const formatted_logs = logs.map((log) => ({
            ...
            module:
                log.module ||
                (log.page
                    ? String(log.page).replace(/^\//, "").split("/")[0]
                    : null),
            action: log.action,
            ...
            status:
                typeof log.status === "number"
                    ? log.status === 1
                        ...
                        : 0
                    : log.extraData &&
                        (log.extraData.error || log.extraData.error_message)
                      ? 0
                      : 1,
            operation_time: log.operationTime
                ? new Date(log.operationTime)
        ...
        throw new ApiError(
            httpStatus.SERVICE_UNAVAILABLE,
            "Failed to batch log user actions",
        );
    }
}
...
async function getUserActionStats(options = {}) {
    try {
        ...
        const time_field = OperationLogModel?.rawAttributes?.operation_time
            ? "operation_time"
            : "created_at";
        ...
        throw new ApiError(
            httpStatus.SERVICE_UNAVAILABLE,
            "Failed to get user action stats",
        );
    }
}
