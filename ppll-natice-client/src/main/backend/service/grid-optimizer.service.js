...
const calculateRiskLevel = (
    ...
    const volatility_score =
        volatility_level === "高" ? 0.8 : volatility_level === "中" ? 0.5 : 0.2;
...
const calculateSupportResistance = (
    ...
    const identify_result = identifySupportResistance({
        symbol,
        kline_list,
        dailyKlineList,
        atr,
...
const optimizeForProfit = (params) => {
    const {
        ...
        total_capital,
        interval_config,
        min_trade_value: userMinTradeValue,
    ...
    const adjustToPrecision = (value, isPrice = false) => {
        ...
        const step = isPrice
            ? precision_rules.tickSize
            : precision_rules.stepSize;
        const minVal = isPrice
            ? precision_rules.minPrice
            : precision_rules.minQty;

        // 防御性检查
        if (!step || step <= 0 || isNaN(step)) {
            console.warn(
                `[adjustToPrecision] Invalid step: ${step}, returning original value: ${value}`,
            );
            return value;
        }
    ...
    for (let atrMultiple = 0.3; atrMultiple <= 5; atrMultiple += 0.1) {
        ...
        const freq_per_kline = estimateTradeFrequency(
            kline_list,
            grid_spacing,
            support,
            resistance,
        ...
        // 遍历不同的每笔交易金额
        for (
            let trade_value = min_trade_value;
            ...
            if (daily_profit > max_daily_profit) {
                ...
                best_config = {
                    ...
                    grid_spacing_percent:
                        new BigNumber((grid_spacing / avg_price) * 100).toFixed(
                            4,
                        ) + "%",
                    trade_quantity: smartFormat(
                        trade_quantity,
                        precision_rules,
                        false,
                    ),
                    trade_value: new BigNumber(trade_value).toFixed(2),
                    expected_daily_frequency: new BigNumber(
                        daily_frequency,
                    ).toFixed(2),
                    expected_daily_profit: new BigNumber(daily_profit).toFixed(
                        2,
                    ),
                    expected_daily_fee: new BigNumber(daily_fee).toFixed(2),
                    expected_daily_roi:
                        new BigNumber(daily_roi * 100).toFixed(4) + "%",
                    single_net_profit: smartFormat(
                        net_profit,
                        precision_rules,
                        true,
                        market.avg_price,
                    ...
                    turnover_ratio:
                        new BigNumber(turnover_ratio * 100).toFixed(2) + "%",
                };
            }
        }
    }
    ...
    if (config_list.length > 0) {
        const top_list = config_list
            ...
            .map((config) => ({
                ...
                grid_spacing_percent:
                    new BigNumber(config.grid_spacing_percent).toFixed(4) + "%",
                trade_quantity: smartFormat(
                ...
                expected_daily_profit: new BigNumber(
                    config.daily_profit,
                ).toFixed(2),
                expected_daily_roi:
                    new BigNumber(config.daily_roi * 100).toFixed(4) + "%",
                single_net_profit: smartFormat(
                    config.net_profit,
                    precision_rules,
                    true,
                ...
                turnover_ratio:
                    new BigNumber(config.turnover_ratio * 100).toFixed(2) + "%",
            }));
        analysis = {
...
const optimizeForCostReduction = (params) => {
    ...
    for (let atrMultiple = 0.1; atrMultiple <= 2; atrMultiple += 0.05) {
        ...
        for (
            ...
            // 亏损惩罚：如果亏损，降低效率评分
            const lossPenalty =
                daily_profit < 0 ? Math.abs(daily_profit) / daily_fee : 0;
    ...
    if (config_list.length > 0) {
        const top_list = config_list
            ...
            .map((config) => ({
                ...
                grid_spacing_percent:
                    new BigNumber(
                        (config.grid_spacing / avg_price) * 100,
                    ).toFixed(4) + "%",
                trade_quantity: smartFormat(
                    config.trade_quantity,
                    precision_rules,
                ...
                expected_daily_roi:
                    new BigNumber(
                        (config.daily_profit / total_capital) * 100,
                    ).toFixed(4) + "%",
                single_net_profit: smartFormat(
                    config.net_profit,
                    precision_rules,
    ...
    for (let atrMultiple = 10; atrMultiple >= 0.5; atrMultiple -= 0.2) {
        ...
        if (daily_frequency < min_daily_frequency) {
            ...
            best_config = {
                ...
                grid_spacing_percent:
                    new BigNumber((grid_spacing / avg_price) * 100).toFixed(4) +
                    "%",
                trade_quantity: smartFormat(
                    trade_quantity,
                    precision_rules,
...
const optimizeGridParams = async (options) => {
    ...
    if (!total_capital || total_capital <= 0)
        throw new Error("总资金必须大于0");
    if (!api_key || !api_secret) throw new Error("缺少API凭证");

    const interval_config = INTERVAL_MAP[interval];
    if (!interval_config) {
        throw new Error(
            `不支持的K线周期: ${interval}，支持: ${Object.keys(INTERVAL_MAP).join(", ")}`,
        );
    }

    // 2. 获取交易所信息并验证精度规则
    const exchangeInfo =
        await binanceExchangeInfoService.getLatestExchangeInfo();
    if (!exchangeInfo) {
    ...
    const effectiveMinQty = Math.max(
        minQty,
        !isNaN(marketMinQty) ? marketMinQty : 0,
    );
    ...
    try {
        ...
        console.warn(
            `[警告] 获取实时标记价格失败，将仅使用 MIN_NOTIONAL 校验: ${error?.message || error}`,
        );
    }
    ...
    const minNotionalRaw =
        minNotionalFilter?.notional ?? minNotionalFilter?.minNotional;
    ...
    if (requiredMinTradeValue > 0) {
        if (
            min_trade_value < requiredMinTradeValue ||
            max_trade_value < requiredMinTradeValue
        ) {
            const minTradeValueText = new BigNumber(
                requiredMinTradeValue,
            ).toFixed(2);
            throw new Error(
                `根据币安官方要求，${symbol} 最小单笔交易金额为 ${minTradeValueText} USDT（基于最小数量 ${requiredMinQty.toFixed(stepSize > 0 ? Math.max(0, Math.floor(-Math.log10(stepSize))) : 3)} BTC 与当前价格 ${new BigNumber(current_price).toFixed(2)} USDT 计算），请调整每笔交易金额范围`,
            );
        }
    }

    const adjusted_min_trade_value = min_trade_value;
    const adjusted_max_trade_value = Math.max(
        max_trade_value,
        adjusted_min_trade_value,
    );
    ...
    try {
        ...
        console.warn(
            `[警告] 获取日K线数据失败，将不使用多周期确认: ${error?.message || error}`,
        );
    }
    ...
    const market = calculateSupportResistance(
        kline_list,
        dailyKlineList,
        symbol,
        atr,
    ...
    const grid_spacing_percent =
        recommended?.analysis?.top_list?.[0]?.grid_spacing_percent || 0;
    ...
    return {
        ...
        optimize_target,
        optimize_target_label:
            optimize_target === "profit" ? "收益最大化" : "成本摊薄",
        ...
        // 市场分析数据
        market: {
            current_price: smartFormat(
                market.currentPrice,
                precision_rules_for_return,
                true,
                market.currentPrice,
            ),
            support: smartFormat(
            ...
            resistance: smartFormat(
                market.resistance,
                precision_rules_for_return,
                true,
                market.currentPrice,
            ),
            avg_price: smartFormat(
                market.avg_price,
                precision_rules_for_return,
                true,
                market.currentPrice,
            ),
            price_range: smartFormat(
                market.price_range,
                precision_rules_for_return,
                true,
                market.currentPrice,
