...
const init = (server, wsManagerInstance) => {
    ...
    io.on("connection", (socket) => {
        ...
        socket.on("subscribe_user_stream", async (data) => {
            ...
            UtilRecord.log(
                `[SocketIO] 收到 subscribe_user_stream 请求, apiKey: ${apiKey ? apiKey.substring(0, 8) + "..." : "undefined"}, market: ${market}`,
            );

            if (!apiKey || !apiSecret) {
                socket.emit("error", {
                    message: "缺少必要参数: apiKey, apiSecret",
                });
                return;
            }
            ...
            try {
                ...
                socket.emit("user_stream_status", {
                    status: "connected",
                    apiKey,
                    market,
                });
                UtilRecord.log(
                    `[SocketIO] Socket ${socket.id} 订阅用户数据流 ${subKey} 成功`,
                );
                UtilRecord.log(
                    `[SocketIO] Socket 已加入房间: user:${apiKey}:${market}`,
                );
            } catch (err) {
                UtilRecord.log(
                    `[SocketIO] 订阅用户数据流失败 ${subKey}: ${err?.message || err}`,
                );
                socket.emit("error", {
                    message: `订阅失败: ${err?.message || err}`,
                });
                socket.leave(room);
        ...
        socket.on("unsubscribe_user_stream", (data) => {
            ...
            if (apiKey && socket.subscribedUserStreamList.has(subKey)) {
                ...
                socket.subscribedUserStreamList.delete(subKey);
                UtilRecord.log(
                    `[SocketIO] Socket ${socket.id} 取消订阅用户数据流 ${subKey}`,
                );
            }
        });
        ...
        socket.on("subscribe_ticker", (data) => {
            ...
            if (socket.subscribedTickerList.has(subKey)) {
                UtilRecord.log(
                    `[SocketIO] Socket ${socket.id} 已订阅 Ticker ${subKey}，跳过重复订阅`,
                );
                socket.emit("ticker_status", {
                    status: "connected",
                    symbol,
                    market,
                });
            ...
            // 订阅标记价格
            UtilRecord.log(
                `[SocketIO] 调用 wsManager.subscribeMarkPrice(${symbol})`,
            );
            ...
            UtilRecord.log(
                `[SocketIO] Socket ${socket.id} 订阅 Ticker ${subKey} 成功, 房间: ${room}`,
            );
        });
        ...
        socket.on("unsubscribe_ticker", (data) => {
            ...
            if (symbol && socket.subscribedTickerList.has(subKey)) {
                ...
                UtilRecord.log(
                    `[SocketIO] Socket ${socket.id} 取消订阅 Ticker ${subKey}`,
                );
            }
        });
        ...
        socket.on("switch_ticker", (data) => {
            ...
            if (oldSymbol) {
                ...
                if (oldSubKey === newSubKey) {
                    UtilRecord.log(
                        `[SocketIO] Socket ${socket.id} 切换 Ticker 新旧相同 ${oldSymbol}，跳过`,
                    );
                    socket.emit("ticker_status", {
                        status: "connected",
                        symbol: newSymbol,
            ...
            if (newSymbol && newSubKey) {
                ...
                if (socket.subscribedTickerList.has(newSubKey)) {
                    UtilRecord.log(
                        `[SocketIO] Socket ${socket.id} 已订阅 Ticker ${newSubKey}，跳过重复订阅`,
                    );
                    socket.emit("ticker_status", {
                        status: "connected",
                        symbol: newSymbol,
                ...
                UtilRecord.log(
                    `[SocketIO] Socket ${socket.id} 切换 Ticker ${oldSymbol || "无"} -> ${newSymbol}`,
                );
            }
        });
        ...
        socket.on("frontend_logs", async (data) => {
            try {
                ...
                UtilRecord.trace(
                    `[SocketIO] 处理前端日志失败: ${error.message}`,
                );
            }
        });
    ...
    if (!wsManager.listenerCount("userDataUpdate")) {
        wsManager.on("userDataUpdate", ({ apiKey, market, data }) => {
            ...
            UtilRecord.log(
                `[SocketIO] 收到 userDataUpdate，转发到房间 ${room}, eventType: ${data.eventType}, 房间内客户端数: ${roomSize}`,
            );
            io.to(room).emit("account_update", data);
            UtilRecord.log(
                `[SocketIO] 已发送 account_update 事件到房间 ${room}, 数据摘要:`,
                JSON.stringify({
                    eventType: data.eventType,
                    transactionTime: data.transactionTime,
                    accountsCount:
                        data.updateData?.updatedBalances?.length || 0,
                    positionsCount:
 ...
 */
const emitStrategyStatusUpdate = (
    strategyId,
    executionStatus,
    extraData = {},
) => {
    ...
    if (!IS_PRODUCTION) {
        UtilRecord.log(
            `[SocketIO] 推送策略状态更新: strategyId=${strategyId}, status=${executionStatus}`,
        );
    }
};
