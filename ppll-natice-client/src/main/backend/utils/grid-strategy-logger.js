    ...
    _handleWriteError(error, message, event_type) {
        ...
        if (this._errorCount >= this._errorAlertThreshold) {
            ...
            if (timeSinceLastError <= this._errorAlertWindow) {
                ...
                console.error(
                    `${prefix} [数据库写入告警] 5分钟内已发生${this._errorCount}次写入失败，最近错误: ${this._lastErrorMessage}`,
                );
        ...
        if (this._errorCount % 10 === 0) {
            const prefix = this.generateLogPrefix("ERROR");
            console.error(
                `${prefix} [数据库写入失败] 第${this._errorCount}次失败 | 事件类型: ${event_type} | 消息: ${message} | 错误: ${this._lastErrorMessage}`,
            );
        }
    }
    ...
    log(...messageList) {
        const message =
            messageList.length > 0
                ? this.formatMessageList(messageList)
                : this._lastMessage;
        if (message) {
