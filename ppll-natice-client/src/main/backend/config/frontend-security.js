...
const FRONTEND_BACKEND_SECURITY = {
    ...
    ENABLE_LOCALHOST_BYPASS:
        process.env.ENABLE_LOCALHOST_BYPASS === "true" || false,

    // 管理令牌头部名称
    MANAGEMENT_TOKEN_HEADER:
        process.env.MANAGEMENT_TOKEN_HEADER || "x-management-token",

    // 管理令牌（用于真正的内部服务调用）
    MANAGEMENT_TOKEN: process.env.MANAGEMENT_TOKEN || generateSecureToken(),

    // 是否启用前端代理攻击检测
    ENABLE_FRONTEND_PROXY_DETECTION:
        process.env.ENABLE_FRONTEND_PROXY_DETECTION !== "false",
    ...
    // API路径模式（需要严格限流的接口）
    API_PATH_PATTERNS: ["/api/", "/v1/", "/v2/", "/graphql", "/webhook"],
};
