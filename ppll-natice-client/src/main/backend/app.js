...
const logApiKeysStatus = async (phase) => {
    try {
        const [result] = await db.sequelize.query(
            "SELECT COUNT(*) as count, SUM(CASE WHEN deleted=1 THEN 1 ELSE 0 END) as deleted_count FROM binance_api_keys",
        );
        const [allResult] = await db.sequelize.query(
            "SELECT id, name, deleted, status, created_at FROM binance_api_keys ORDER BY id",
        );
        const ids = allResult.map((r) => r.id).join(",");
        const names = allResult.map((r) => r.name).join(",");
        const deletedFlags = allResult
            .map((r) => `${r.id}:${r.deleted}`)
            .join(",");
        console.log(
            `[API-KEY-DEBUG] ${phase} total=${result[0].count} deleted=${result[0].deleted_count}`,
        );
...
const cleanBackupTables = async () => {
    ...
    console.log(
        `[API-KEY-DEBUG] BACKUP_TABLES_COUNT=${backups.length} names=${backups.map((b) => b.name).join(",")}`,
    );
...
function killProcessOnPort(port) {
    try {
        ...
        if (pids.length > 0) {
            console.log(
                `[端口清理] 发现端口 ${port} 被占用，正在清理进程: ${pids.join(", ")}`,
            );
            pids.forEach((pid) => {
                try {
                    if (platform === "win32") {
...
const server = app.listen(port, () => {
    ...
    try {
        ...
        global.wsManager.on("error", (errorData) => {
            ...
            UtilRecord.log(
                `[WSManager Global] WebSocket 错误事件:`,
...
if (global.GIT_INFO) {
    ...
    console.log(
        "Git信息已存储到global.gitInfo，可在service目录下的所有文件中访问",
    );
} else {
