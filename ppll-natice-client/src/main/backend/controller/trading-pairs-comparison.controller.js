...
const getFuturesOnlyPairs = catchAsync(async (req, res) => {
    ...
    const result =
        await tradingPairsComparisonService.getFuturesOnlyPairs(suffix);
...
const getComprehensiveReport = catchAsync(async (req, res) => {
    const { suffix } = req.query;
    const result =
        await tradingPairsComparisonService.getComprehensiveReport(suffix);
...
const getSpotTradingPairs = catchAsync(async (req, res) => {
    const { suffix } = req.query;
    const pairs =
        await tradingPairsComparisonService.fetchSpotTradingPairs(suffix);
...
const getFuturesTradingPairs = catchAsync(async (req, res) => {
    const { suffix } = req.query;
    const pairs =
        await tradingPairsComparisonService.fetchFuturesTradingPairs(suffix);
    return res.apiSuccess(
        {
            count: pairs.length,
            pairs: pairs.sort(),
            description: suffix
                ? `以${suffix}结尾的合约交易对（永续合约）`
                : "所有合约交易对（永续合约）",
        },
        "成功获取合约交易对列表",
    );
...
const getCoinMFuturesTradingPairs = catchAsync(async (req, res) => {
    const pairs =
        await tradingPairsComparisonService.fetchCoinMFuturesTradingPairs();
    return res.apiSuccess(
        {
            count: pairs.length,
            pairs: pairs.sort(),
