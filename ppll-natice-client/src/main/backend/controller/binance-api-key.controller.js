...
const deleteApiKey = catchAsync(async (req, res) => {
    const clientIp =
        ...
        "-";
    console.log(
        `[API-KEY-DEBUG] DELETE_REQUEST id=${req.body.id} ip=${clientIp} user-agent=${req.get("user-agent")?.substring(0, 50)}...`,
    );
...
const queryApiKey = catchAsync(async (req, res) => {
    ...
    if (idList.length > 0) {
        ...
        if (currentPage > 1)
            options.offset = (Number(currentPage) - 1) * Number(pageSize);
    }

    let api_key_list = await binanceApiKeyService.getAllApiKeys(
        filter,
        options,
    );
    ...
    return res.apiSuccess(
        {
            ...
            pagination: {
                ...
                pageSize:
                    idList.length > 0 ? api_key_list.length : Number(pageSize),
                total: total,
            },
        },
        "查询 ApiKey 成功",
