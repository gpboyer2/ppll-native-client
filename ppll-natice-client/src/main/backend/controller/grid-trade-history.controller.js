...
const query = catchAsync(async (req, res) => {
    const { api_key, currentPage, pageSize, ...filters } = req.query || {};

    // 强制把 api_key 放入过滤，避免越权
    const resp = await service.getAllTradeHistories(
        { ...filters, api_key },
        {
...
/**
 * 获取详情（需提供 api_key/api_secret，且只能查看自己的记录）
 */
