...
const getSpotAccount = catchAsync(async (req, res) => {
    let { api_key, api_secret, includeEmptyBalances } =
        extractApiCredentials(req);
...
const setLeverage = catchAsync(async (req, res) => {
    let { api_key, api_secret, leverage_list, delay } =
        extractApiCredentials(req);
    ...
    if (!leverage_list || !Array.isArray(leverage_list)) {
        return res.apiError(
            null,
            "leverage_list 必须是一个数组，格式为 [{symbol: 'BTCUSDT', leverage: 20}, {symbol: 'ETHUSDT', leverage: 10}]",
        );
    }
    ...
    for (const item of leverage_list) {
        if (!item || typeof item !== "object") {
            return res.apiError(
                null,
                "数组中的每个元素必须是包含 symbol 和 leverage 属性的对象",
            );
        }
        ...
        if (
            !Number.isInteger(item.leverage) ||
            item.leverage < 1 ||
            item.leverage > 125
        ) {
            return res.apiError(
                null,
                `${item.symbol} 的杠杆倍数必须是 1-125 之间的整数`,
            );
        }
    }
...
const generateListenKey = catchAsync(async (req, res) => {
    ...
    try {
        const listen_key =
            await binanceAccountService.generateListenKey(api_key);
