...
export class Performance {
   ...
   * 函数执行次数统计
   */
  static createCounter<T extends (...args: any[]) => any>(fn: T, name: string = fn.name): T {
    let count = 0
...
export class BatchProcessor<T> {
  ...
  constructor(processor: (items: T[]) => void, batchSize: number = 100, waitTime: number = 100) {
...
export class ResourcePool<T> {
  ...
  constructor(factory: () => T, destroyer?: (resource: T) => void, maxSize: number = 10) {
