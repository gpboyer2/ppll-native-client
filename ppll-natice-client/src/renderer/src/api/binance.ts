...
export class BinanceApi {
   ...
   * @returns 账户详细信息
   */
  static async getUmAccountInfo(
    request: AccountInfoRequest
  ): Promise<Response<BinanceApiResponse<AccountInfo>>> {
   ...
   * @returns 开仓操作结果
   */
  static async umOpenPosition(
    request: UmOpenPositionRequest
   ...
   * @returns 平仓操作结果
   */
  static async umClosePosition(
    request: UmClosePositionRequest
  ): Promise<Response<PositionOperationResponse>> {
    return RequestWrapper.post<PositionOperationResponse>(
   ...
   * @returns 是否有效
   */
  static validatePositionConfigs(
    positions: Array<{ symbol: string; long_amount: number; short_amount: number }>
  ): boolean {
