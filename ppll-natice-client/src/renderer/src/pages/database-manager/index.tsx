...
function DatabaseManagerPage() {
  ...
  const handle_rename_column = async () => {
    ...
    const response = await ApiEndpoints.renameColumn(
      selectedTable,
      renameColumnOld,
      renameColumnNew.trim()
    )
    if (response.status === 'success') {
      notifications.show({
        title: '重命名成功',
                ...
                >
                  ...
                  <div className="table-item-meta">
                    ...
                    <div
                      ...
                      ref={showTableMenu === table.name ? table_menu_ref : undefined}
                    >
                      <button
                        className="btn-icon"
                        onClick={(e) => {
                  ...
                  <div className="table-data-container">
                    {selectedRowList.length > 0 && (
                      <div className="batch-actions-bar">
                        <span className="batch-actions-info">
                          已选中 {selectedRowList.length} 条记录
                        </span>
                        <button
                              ...
                              return (
                                <tr key={idx} className={is_selected ? 'selected-row' : ''}>
                                  ...
                                  {tableData.columns.map((col, colIndex) => (
                                    <td
                                      key={col}
                                      className={colIndex === 0 ? 'fixed-column-first' : ''}
                                    >
                                  ...
                                  <td className="fixed-column-last">
                                    <button
                                      ...
                                      onClick={() => {
                                        const pk_column = tableDetail?.columns.find(
                                          (c) => c.primaryKey
                                        )
                    ...
                    {tableData.pagination.total > 0 && (
                      <div className="pagination">
                        ...
                        <span className="pagination-info">
                          第 {currentPage} / {Math.ceil(tableData.pagination.total / pageSize)}{' '}
                          页，共 {tableData.pagination.total} 条
                        </span>
                        <button
                          className="btn btn-sm btn-outline"
