    ...
    <div className="container">
            ...
            {enabled_plugin_list.length > 0 ? (
              <div className="flex flex-col gap-8">
                {enabled_plugin_list.map((plugin) => (
                  <div
                    key={plugin.id}
                    className="flex items-center space-between p-8 rounded border"
                  >
              ...
              <div className="text-muted empty-state">
                ...
                <div>暂无启用的插件</div>
                <Link to="/plugins" className="btn btn-outline mt-8">
                  前往启用
                </Link>
              </div>
            )}
                ...
                {recent_notifications.map((notification) => (
                  ...
                  >
                    <div className="flex items-center gap-8 mb-4">
                      <span
                        className={`tag ${notification.level === 'error' ? 'danger' : notification.level === 'warn' ? 'warn' : 'success'}`}
                      >
                        {notification.level}
                      </span>
                    </div>
      ...
      <section className="surface p-16 mt-16">
          ...
          {pluginList.map((plugin) => {
            ...
            return (
              <Link
                ...
                onMouseEnter={(e) =>
                  !is_disabled && (e.currentTarget.style.transform = 'translateY(-2px)')
                }
                onMouseLeave={(e) =>
                  !is_disabled && (e.currentTarget.style.transform = 'translateY(0)')
                }
              >
                {is_disabled && !is_coming_soon && (
                  <div className="permission-tooltip">插件无权限</div>
                )}
                <div className="card-content" style={{ textAlign: 'center' }}>
                  ...
                  <div
                    className={`tag ${is_coming_soon ? 'warn' : plugin.enable ? 'success' : ''}`}
        ...
        {pluginList.length > 4 && (
          <div
            className="text-muted"
            style={{ fontSize: 'var(--text-xs)', textAlign: 'center', marginTop: '8px' }}
          >
            ← 左右滑动查看更多插件 →
          </div>
        )}
