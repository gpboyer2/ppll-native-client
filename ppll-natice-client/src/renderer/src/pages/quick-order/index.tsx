...
import { AccountInfoCard, type AccountInfoCardRef } from './components/account-info-card'
...
function QuickOrderPage() {
  ...
  const [trading_pair, setTradingPair] = useState(
    () => Storage.get<string>('quick_order_trading_pair', 'BTCUSDT') || 'BTCUSDT'
  )
  ...
  const account_info_card_ref = useRef<AccountInfoCardRef>(null)
  const profit_stats_card_ref = useRef<ProfitStatsCardRef>(null)
      ...
      try {
        ...
        if ('datum' in result && result.datum) {
          handlePositionResponse(
            result.datum as PositionOperationResponse,
            (msg, status) => {
              notifications.update({
                id,
    ...
    try {
      ...
      if (response.status === 'success' && response.datum) {
        ...
        let error_type:
          ...
          | 'signature_error'
          | 'invalid_api_key'
          | 'ip_restricted' = 'validation_failed'
  ...
  const handleRefresh = useCallback(async () => {
    await Promise.all([
      account_info_card_ref.current?.refresh(),
        ...
        const handleAccountUpdate = (data: any) => {
          ...
          if (data.eventType === 'ACCOUNT_UPDATE') {
            ...
            if (accounts && accounts.length > 0) {
              ...
              if (usdt_account) {
                const has_available_balance =
                  usdt_account.availableBalance !== undefined &&
                  usdt_account.availableBalance !== null
                const has_margin_balance =
                  usdt_account.marginBalance !== undefined && usdt_account.marginBalance !== null
                const wallet_balance_raw =
                  usdt_account.walletBalance ??
                  usdt_account.crossWalletBalance ??
                  usdt_account.balance
                const has_wallet_balance =
                  wallet_balance_raw !== undefined && wallet_balance_raw !== null

                available_balance = has_available_balance
                  ? parseFloat(usdt_account.availableBalance || '0')
                  : null
                margin_balance = has_margin_balance
                  ? parseFloat(usdt_account.marginBalance || '0')
                  : null
            ...
            // 过滤有效持仓（持仓数量不为0）
            const valid_positions = (positions || [])
              .filter((p: any) => {
              ...
              .map((p: any) => {
                ...
                const fallbackNotional =
                  Number.isFinite(positionAmtValue) && Number.isFinite(entryPriceValue)
                ...
                return {
                  ...
                  unrealizedProfit:
                    p.unrealisedPnl !== undefined ? p.unrealisedPnl : p.unrealizedProfit || '0',
            ...
            setAccountData((prev) => {
              const newAccountData = {
                available_balance:
                  available_balance !== null ? available_balance : prev.available_balance,
  ...
  const short_positions = account_data.positions.filter(
    (p) =>
      parseFloat(p.positionAmt) < 0 && (p.positionSide === 'SHORT' || p.positionSide === 'BOTH')
  )

  const total_long_amount = long_positions.reduce(
    (sum, p) => sum + Math.abs(parseFloat(p.notional)),
    0
  )
  const total_short_amount = short_positions.reduce(
    (sum, p) => sum + Math.abs(parseFloat(p.notional)),
    0
  )
  ...
  const current_pair_long_amount = useMemo(() => {
    const long_positions = current_pair_positions.filter(
      (p) =>
        parseFloat(p.positionAmt) > 0 && (p.positionSide === 'LONG' || p.positionSide === 'BOTH')
    )
  ...
  const current_pair_short_amount = useMemo(() => {
    const short_positions = current_pair_positions.filter(
      (p) =>
        parseFloat(p.positionAmt) < 0 && (p.positionSide === 'SHORT' || p.positionSide === 'BOTH')
    )
  ...
  const current_pair_total_short_amount = useMemo(() => {
    return current_pair_short_positions.reduce(
      (sum, p) => sum + Math.abs(parseFloat(p.notional)),
      0
    )
  ...
  const handleOpenPosition = async (side: 'long' | 'short', amount: number) => {
    ...
    if (account_data.available_balance < amount) {
      showMessage(
        `可用保证金不足，当前: ${account_data.available_balance.toFixed(2)} USDT`,
        'error'
      )
      return
    }
  ...
  const handleCloseByPercentage = async (side: CloseSide, percentage: number) => {
    ...
    showCloseConfirm(`确认平仓`, `确定要${position_text}平${side_text}单吗？`, async () => {
      await executeWithNotification(
  ...
  const handleBalanceByOpenPosition = async () => {
    ...
    await executeWithNotification(`开仓持平 ${trading_pair}`, async () => {
      const positions: Array<{ symbol: string; side: 'LONG' | 'SHORT'; amount: number }> = []
  ...
  const handleBalanceByClosePosition = async () => {
    ...
    await executeWithNotification(`平仓持平 ${trading_pair}`, async () => {
      ...
      if (response.status === 'success' && response.datum) {
        // 异步刷新持仓数据，不阻塞通知更新
        account_info_card_ref.current?.refresh().catch((err) => {
          console.error('[QuickOrder] 刷新持仓数据失败:', err)
        })
        return { datum: response.datum as PositionOperationResponse }
      }
  ...
  const other_pairs = usdt_pairs.filter((pair) => !quick_pair_options.includes(pair)).slice(0, 20)
  ...
  return (
    <div className="container">
      ...
      {/* 账户验证卡片 */}
      <AccountValidationCard account_data={account_validation_result ?? validation_result} />
      ...
      <div className="quick-order-card">
        ...
        <div className="quick-order-card-content">
          ...
          <div className="quick-order-card-right">
            <AccountInfoCard
              ref={account_info_card_ref}
              externalData={account_data}
              onRefreshRequest={loadAccountData}
            />
            <ProfitStatsCard ref={profit_stats_card_ref} />
          </div>
        </div>
      </div>
      ...
      <PositionFloatingPanel
        ...
        on_close_success={() => {
          profit_stats_card_ref.current?.refresh().catch((err) => {
            console.error('[QuickOrder] 刷新数据失败:', err)
          })
        }}
      />
