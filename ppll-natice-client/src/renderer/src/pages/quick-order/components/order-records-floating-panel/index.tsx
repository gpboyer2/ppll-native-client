...
function OrderRecordsFloatingPanel(
  props: OrderRecordsFloatingPanelProps,
  ref: React.Ref<OrderRecordsFloatingPanelRef>
) {
  const {
    get_active_api_key,
    show_message,
      ...
      try {
        ...
        if (response.status === 'success') {
          setOrderRecords((prev) =>
            prev.map((r) => (r.id === record.id ? { ...r, is_collapsed: new_collapsed_state } : r))
          )
        } else {
  ...
  return (
    ...
    >
      <div className="order-records-floating-panel-header" onMouseDown={handleMouseDown}>
        ...
        <button className="order-records-floating-panel-close" onClick={handleClose} title="关闭">
          <IconX size={16} />
        </button>
      </div>
                ...
                <div key={record.id} className="order-records-floating-panel-item">
                  <div className="order-records-floating-panel-item-main">
                    <div className="order-records-floating-panel-symbol">
                      {record.symbol}
                      <span className="order-records-floating-panel-order-id">
                        {record.order_id}
                      </span>
                    </div>
                    <div
                      className={`order-records-floating-panel-side ${is_long ? 'side-long' : 'side-short'}`}
                    >
                      {side}
                    </div>
                  </div>
                  ...
                  {!record.is_collapsed && (
                    <div className="order-records-floating-panel-item-details">
                      <div className="order-records-floating-panel-detail-row">
                        ...
                        <span className="order-records-floating-panel-value">
                          {executed_price.toFixed(2)}
                        </span>
                      </div>
                      <div className="order-records-floating-panel-detail-row">
                        <span className="order-records-floating-panel-label">持仓额</span>
                        <span
                          className={`order-records-floating-panel-value ${is_long ? 'value-long' : 'value-short'}`}
                        >
