...
class WebSocketConnectionManager extends EventEmitter {
    ...
    _createWsConfig(verbose = true) {
        ...
        // 添加连接超时配置
        const wsOptions =
            !IS_PRODUCTION && this.options.enableProxyNonProd && ws_proxy
                ? {
        ...
        const logger = {
            info: verbose
                ? (...args) => UtilRecord.log("[WSClient] info:", ...args)
                : silentLogger,
            ...
            debug: IS_PRODUCTION
                ? silentLogger
                : (...args) => UtilRecord.debug("[WSClient] debug:", ...args),
    ...
    _checkConnectionTimeout(clientKey) {
        ...
        if (elapsed > CONNECTION_TIMEOUT && !state.reconnectTimer) {
            UtilRecord.log(
                `[WSManager] 连接可能已超时 (${clientKey})，上次连接: ${elapsed}ms 前`,
            );
        }
    }
        ...
        client.on("formattedMessage", (data) => {
            try {
                ...
                messageList.forEach((message) => {
                    ...
                    const eventType =
                        message.eventType || message.event_type || message.e;
                    const markPrice =
                        message.markPrice ?? message.mark_price ?? message.p;
                    const hasMarkPrice =
                        message.symbol && markPrice !== undefined;
                ...
                UtilRecord.log(
                    `[WSManager] formattedMessage dispatch error: ${e.message}`,
                );
            }
        });
    ...
    _decrRef(key) {
        ...
        if (next === 0) this.subRefs.delete(key);
        else this.subRefs.set(key, next);
        return next;
    }
    ...
    subscribeContinuousKlines(
        ...
        if (ref > 1) {
            UtilRecord.log(
                `[WSManager] skip duplicate subscribe ${subKey} (ref=${ref})`,
            );
            return;
        }
        const client = this._getClient({ market, scope });
        client.subscribeContinuousContractKlines(
            symbol,
            contractType,
            interval,
            "usdm",
    ...
    unsubscribeMarkPrice(symbol) {
        ...
        UtilRecord.log(`[WSManager] unsubscribeMarkPrice 引用计数`, {
            symbol,
            subKey,
            before: currentRef,
            after: left,
        ...
        if (left > 0) {
            UtilRecord.log(`[WSManager] 跳过取消订阅，仍有其他订阅者`, {
                symbol,
                remaining: left,
            });
            return;
        ...
        try {
            ...
            UtilRecord.log(
                `[WSManager] unsubscribed ${subKey} (topic: ${topic})`,
            );
        } catch (e) {
            UtilRecord.log(
                `[WSManager] unsubscribeMarkPrice failed: ${e.message}`,
            );
        }
    }

    unsubscribeContinuousKlines(
        symbol,
        interval = "1m",
        contractType = "perpetual",
    ) {
        ...
        try {
            ...
            UtilRecord.log(
                `[WSManager] unsubscribeContinuousKlines failed: ${e.message}`,
            );
        }
    }
    ...
    async subscribeUserData(apiKey, apiSecret, market = "usdm") {
        ...
        UtilRecord.log(
            `[WSManager] 开始订阅用户数据流, apiKey: ${apiKey.substring(0, 8)}..., market: ${market}`,
        );
        ...
        if (this.userDataSubs.has(subKey)) {
            ...
            sub.refCount += 1;
            UtilRecord.log(
                `[WSManager] 复用已有用户数据流订阅 ${subKey} (refCount=${sub.refCount})`,
            );
            return;
        }
        ...
        websocketClient.on("formattedMessage", (data) => {
            try {
                ...
                UtilRecord.log(
                    `[WSManager] 收到 formattedMessage: ${subKey}, eventType: ${data.eventType || "undefined"}`,
                );
                ...
                if (
                    ...
                    UtilRecord.log(
                        `[WSManager] 用户数据更新 ${subKey}: ${data.eventType}`,
                    );
                    // 打印 ACCOUNT_UPDATE 的详细数据
                    if (data.eventType === "ACCOUNT_UPDATE") {
                        const updateData = data.updateData || {};
                        const balancesCount =
                            updateData.updatedBalances?.length || 0;
                        const positionsCount =
                            updateData.updatedPositions?.length || 0;
                        UtilRecord.log(
                            `[WSManager] ACCOUNT_UPDATE 详情 - 余额数量: ${balancesCount}, 持仓数量: ${positionsCount}`,
                        );
                        if (balancesCount > 0) {
                            UtilRecord.log(
                                `[WSManager] ACCOUNT_UPDATE updatedBalances:`,
                                JSON.stringify(updateData.updatedBalances),
                            );
                        }
                        if (positionsCount > 0) {
                            UtilRecord.log(
                                `[WSManager] ACCOUNT_UPDATE updatedPositions:`,
                                JSON.stringify(updateData.updatedPositions),
                            );
                        }
                        // 更新缓存
                        try {
                            cacheManager.updateFromWebSocket(
                                apiKey,
                                market,
                                data,
                            );
                            ...
                            UtilRecord.log(
                                `[WSManager] 更新缓存失败: ${cacheError.message}`,
                            );
                        }
                    }
                    UtilRecord.log(
                        `[WSManager] 转发 userDataUpdate 事件, apiKey: ${apiKey.substring(0, 8)}..., market: ${market}`,
                    );
                    this.emit("userDataUpdate", { apiKey, market, data });
                } else if (data.eventType === "listenKeyExpired") {
                    UtilRecord.log(
                        `[WSManager] listenKey 已过期 ${subKey}，需要重新订阅`,
                    );
                }
            } catch (e) {
        ...
        websocketClient.on("open", (data) => {
            UtilRecord.log(
                `[WSManager] 用户数据流 WebSocket 已打开: ${subKey}, wsKey=${data?.wsKey || ""}`,
            );
        });

        websocketClient.on("reconnecting", (data) => {
            UtilRecord.log(
                `[WSManager] 用户数据流 WebSocket 重连中: ${subKey}`,
            );
        });

        websocketClient.on("reconnected", (data) => {
            UtilRecord.log(
                `[WSManager] 用户数据流 WebSocket 已重连: ${subKey}`,
            );
        });
        ...
        websocketClient.on("error", (data) => {
            ...
            UtilRecord.log(
                `[WSManager] 用户数据流 WebSocket 错误 ${subKey}:`,
                JSON.stringify(errorInfo, null, 2),
            );
        ...
        try {
            ...
            UtilRecord.log(
                `[WSManager] 用户数据流订阅失败 ${subKey}: ${e?.message || e}`,
            );
            throw e;
        }
    }
        ...
        if (sub.refCount <= 0) {
            ...
            if (sub.client) {
                try {
                    ...
                    UtilRecord.log(
                        `[WSManager] 关闭用户数据流失败 ${subKey}: ${e.message}`,
                    );
                }
            }
            ...
            UtilRecord.log(
                `[WSManager] 用户数据流引用计数减少 ${subKey} (refCount=${sub.refCount})`,
            );
        }
    }
