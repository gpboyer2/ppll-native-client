...
router.get(
    ...
    smartMoneyFlowController.getKolVcHoldings,
);

// 推特聪明钱共振信号路由 /v1/smart-money-flow/twitter-resonance-signal
router.get(
    "/twitter-resonance-signal",
    vipMiddleware.validateVipAccess,
    smartMoneyFlowController.getTwitterResonanceSignal,
);
...
router.get(
    "/kol-vc-top-list",
    vipMiddleware.validateVipAccess,
    smartMoneyFlowController.getKolVcTopList,
);
...
router.get(
    "/24h-trade-volume",
    vipMiddleware.validateVipAccess,
    smartMoneyFlowController.get24hTradeVolume,
);

// 获取KOL/VC 30日盈亏分布数据 /v1/smart-money-flow/30d-profit-distribution
router.get(
    "/30d-profit-distribution",
    vipMiddleware.validateVipAccess,
    smartMoneyFlowController.get30DayProfitDistribution,
);
