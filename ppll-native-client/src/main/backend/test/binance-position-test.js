...
async function testPositionAPI() {
    ...
    try {
        ...
        if (keyRecord) {
            ...
            console.log(
                `使用 API Key: ${keyRecord.name} (${api_key?.substring(0, 8)}...)`,
            );
        }
    } catch (error) {
    ...
    try {
        ...
        const activePositions =
            account.positions?.filter((p) => parseFloat(p.positionAmt) !== 0) ||
            [];
        ...
        // 找出有持仓的交易对
        const activePositionsV3 = positions.filter(
        ...
        if (activePositionsV3.length > 0) {
            ...
            console.log(
                sprintf(
                    "%-15s %-10s %-15s %-15s %-18s %-15s %-15s %-10s",
                    "交易对",
                    "方向",
                    "持仓量",
                    "开仓均价",
            ...
            console.log(
                "  breakEvenPrice  - 保本价格（包含手续费的开仓成本价，平仓时达到此价格不亏不赚）",
            );
            console.log(
                "  markPrice       - 标记价格（当前市场价格，用于计算未实现盈亏）",
            );
            console.log(
                "  liquidationPrice - 强平价格（价格达到此值会被强制平仓）",
            );
            ...
            console.log(
                "  - 当 markPrice 达到 breakEvenPrice 时，平仓后不亏不赚",
            );
            console.log(
                "  - 这个值已经考虑了所有历史开仓/加仓/减仓操作的平均成本",
            );
            ...
            if (firstPos) {
                ...
                const diff = (
                    ((breakEvenPrice - entryPrice) / entryPrice) *
                    100
                ).toFixed(4);
                console.log(`  示例 ${firstPos.symbol}:`);
                console.log(`    entryPrice: ${entryPrice} (纯开仓均价)`);
                console.log(
                    `    breakEvenPrice: ${breakEvenPrice} (含手续费保本价)`,
                );
