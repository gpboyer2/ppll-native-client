...
const validateStrategyParams = async (req, res, next) => {
    try {
        ...
        // 新字段（做多/做空的开仓/平仓数量）
        if (grid_long_open_quantity)
            quantitiesToValidate.push(grid_long_open_quantity);
        if (grid_long_close_quantity)
            quantitiesToValidate.push(grid_long_close_quantity);
        if (grid_short_open_quantity)
            quantitiesToValidate.push(grid_short_open_quantity);
        if (grid_short_close_quantity)
            quantitiesToValidate.push(grid_short_close_quantity);

        // 验证每个数量字段
        for (const quantity of quantitiesToValidate) {
            const quantityResult = strategyValidator.validateQuantity(
                quantity,
                symbolInfo,
            );
            if (!quantityResult.valid) {
                return res.apiError(
                    null,
                    `${quantityResult.message}${quantityResult.suggestion ? `，建议值: ${quantityResult.suggestion}` : ""}`,
                );
            }
        }
        ...
        if (symbolInfo.bidPrice || symbolInfo.askPrice) {
            ...
            currentPrice = Number(
                symbolInfo.bidPrice && symbolInfo.askPrice
                    ? (Number(symbolInfo.bidPrice) +
                          Number(symbolInfo.askPrice)) /
                          2
                    : symbolInfo.bidPrice || symbolInfo.askPrice,
            );
        }
        ...
        if (!currentPrice) {
            ...
            const markPriceRecord =
                await markPriceService.getMarkPriceBySymbol(trading_pair);
        ...
        if (currentPrice) {
            const minNotionalResult = strategyValidator.validateMinNotional(
            ...
            console.warn(
                `[validateStrategyParams] 无法获取 ${trading_pair} 当前价格，跳过 MIN_NOTIONAL 验证`,
            );
        }

        // 6. 验证价格差价
        if (grid_price_difference) {
            const priceResult = strategyValidator.validatePriceDifference(
                grid_price_difference,
                symbolInfo,
            );
            if (!priceResult.valid) {
                return res.apiError(
                    null,
                    `${priceResult.message}${priceResult.suggestion ? `，建议值: ${priceResult.suggestion}` : ""}`,
                );
            }
        }
