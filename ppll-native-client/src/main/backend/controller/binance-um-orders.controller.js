...
async function checkAccountBalance(api_key, api_secret) {
    try {
        ...
        if (
            new bigNumber(accountInfo.availableBalance).isLessThan(
                MIN_BALANCE_REQUIRED,
            )
        ) {
            throw new Error(
                `当前合约账户余额${accountInfo.availableBalance}, 不足${MIN_BALANCE_REQUIRED}u, 请充值`,
            );
        }
...
const validateParams = (requiredParams) => {
    return (req, res, next) => {
        ...
        if (missingParams.length > 0) {
            return res.apiError(
                null,
                `${missingParams.join(", ")} is not defined`,
            );
        }
...
const umClosePosition = catchAsync(async (req, res) => {
    ...
    const totalRequestPositions = Array.isArray(positions)
        ? positions.length
        : 0;
    UtilRecord.log(
    ...
    // 判断实际是否有仓位被平掉
    if (
        result.validPositions === 0 ||
        (result.processedCount === 0 && result.skippedPositions > 0)
    ) {
        return res.apiError(
            result,
            result.message ||
                `没有可平仓的仓位，共跳过 ${result.skippedPositions} 个`,
        );
    }
...
const setShortTakeProfit = catchAsync(async (req, res) => {
    ...
    return res.apiSuccess(
        result,
        `请等待约 ${positions.length * 0.5} 秒后，在APP查看止盈设置结果`,
    );
});
...
const queryQuickOrderRecords = catchAsync(async (req, res) => {
    ...
    if (api_secret && rows.length > 0) {
        try {
            enriched_rows = await ordersService.enrichQuickOrderRecords(
                rows,
                api_key,
                api_secret,
            );
...
const updateQuickOrderCollapse = catchAsync(async (req, res) => {
    ...
    try {
        await ordersService.updateQuickOrderCollapse({
            api_key,
            order_id,
            is_collapsed,
        });
