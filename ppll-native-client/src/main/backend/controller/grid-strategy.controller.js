...
const list = catchAsync(async (req, res) => {
    ...
    const grid = await gridStrategyService.getAllGridStrategys(
        credentials,
        pagination,
    );
...
const validateNumberParam = (value, name, allowZero = false) => {
    ...
    if (allowZero ? num < 0 : num <= 0)
        return {
            valid: false,
            message: `${name} 必须${allowZero ? "是非负" : "是大于0"}的数字`,
        };
    return { valid: true };
};
...
const create = catchAsync(async (req, res) => {
    ...
    const validations = [
        validateNumberParam(
            req.body.grid_price_difference,
            "grid_price_difference",
        ),
        validateNumberParam(
            req.body.grid_trade_quantity,
            "grid_trade_quantity",
        ),
        validateNumberParam(
            req.body.max_open_position_quantity,
            "max_open_position_quantity",
        ),
        validateNumberParam(
            req.body.min_open_position_quantity,
    ...
    try {
        ...
        if (exchangeInfoRecord && exchangeInfoRecord.exchange_info) {
            ...
            const symbol = exchangeInfo.symbols?.find(
                (s) => s.symbol === req.body.trading_pair,
            );
        ...
        console.error(
            "[grid-strategy.controller] 获取 exchangeInfo 失败，使用默认精度:",
            error,
        );
        strategyData.price_precision = 2;
        strategyData.quantity_precision = 3;
    ...
    try {
        const { row, created } =
            await gridStrategyService.createGridStrategy(strategyData);

        if (!created) {
            return res.apiError(
                null,
                `已存在该交易对 ${req.body.trading_pair} 的网格策略`,
            );
        }
...
const update = catchAsync(async (req, res) => {
    ...
    if (updateData.trading_pair) {
        try {
            ...
            const exchangeInfoRecord = await BinanceExchangeInfo.getLatest();
            if (exchangeInfoRecord && exchangeInfoRecord.exchange_info) {
                const exchangeInfo = JSON.parse(
                    exchangeInfoRecord.exchange_info,
                );
                const symbol = exchangeInfo.symbols?.find(
                    (s) => s.symbol === updateData.trading_pair,
                );

                if (symbol) {
                    updateData.price_precision = symbol.pricePrecision ?? 2;
                    updateData.quantity_precision =
                        symbol.quantityPrecision ?? 3;
                } else {
