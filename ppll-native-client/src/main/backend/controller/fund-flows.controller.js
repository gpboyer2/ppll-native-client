...
const getContractFundFlows = catchAsync(async (req, res) => {
    ...
    const result = await fundFlowsService.getContractFundFlows(
        asset,
        timeframe,
    );
...
const fetchGateIOData = async (params) => {
    try {
        const response = await axios.get(
            `${GATE_API.BASE_URL}/fund_flow/analysis`,
            {
                params,
...
const fetchGateIOPercentage = async (params) => {
    try {
        const response = await axios.get(
            `${GATE_API.BASE_URL}/fund_flow/percentage`,
            {
                params,
                timeout: GATE_API.TIMEOUT,
                headers: {
...
const coinank_fetchOpenInterestHistory = async (params) => {
    try {
        ...
        const response = await axios.get(
            `${COINANK_API.BASE_URL}/openInterest/chart`,
            {
                params,
                ...
                headers: {
                    ...
                    "coinank-apikey":
                        "LWIzMWUtYzU0Ny1kMjk5LWI2ZDA3Yjc2MzFhYmEyYzkwM2NjfDI4NjUwNTkyMDM5NDQzNDc=",
...
const fetchOpenInterestFromPage = async (coin_type) => {
    ...
    try {
        ...
        const perpetualRows = result.filter((r) =>
            r.exchangeAbbr.includes("永续"),
        );
...
const getOpenInterestByExchange = catchAsync(async (req, res) => {
    ...
    return res.apiSuccess(
        data,
        `获取全网交易所持仓量数据成功` +
            `https://www.gate.com/zh/crypto-market-data/funds/futures-open-interest/${coin_type}`,
    );
});
