...
class ExchangePrecisionManager {
    ...
    async getExchangeInfo(options) {
        ...
        if (!forceRefresh && this.isMemoryCacheValid(exchange, marketType)) {
            ...
            UtilRecord.log(
                `[精度管理] 从内存缓存获取 ${exchange} ${marketType} 交易所信息(${cached.exchangeInfo.symbols?.length || 0}个交易对)`,
            );
        ...
        try {
            // 第二级:检查数据库缓存
            if (!forceRefresh) {
                const dbRecord = await this.getFromDatabase(
                    exchange,
                    marketType,
                );
                if (dbRecord) {
                    ...
                    UtilRecord.log(
                        `[精度管理] 从数据库缓存获取 ${exchange} ${marketType} 交易所信息(${dbRecord.symbols?.length || 0}个交易对)`,
                    );
                    ...
                    if (needsUpdate) {
                        UtilRecord.log(
                            `[精度管理] ${exchange} ${marketType} 数据库缓存已过期,启动后台更新任务`,
                        );
            ...
            // 第三级:从API获取
            UtilRecord.log(
                `[精度管理] 内存和数据库均无有效缓存,从 ${exchange} API获取 ${marketType} 交易所信息`,
            );
            const exchangeInfo = await this.fetchFromAPI(
                exchange,
                marketType,
                apiKey,
                apiSecret,
            ...
            if (
                ...
                // 更新数据库缓存(异步,不阻塞主流程)
                this.saveToDatabase(exchange, marketType, exchangeInfo).catch(
                    (err) => {
                        UtilRecord.error(
                            `[精度管理] 保存 ${exchange} ${marketType} 交易所信息到数据库失败:`,
                            err?.message || err,
                        );
                    },
                );
            ...
            // 所有方式都失败
            UtilRecord.error(
                `[精度管理] 无法通过任何方式获取 ${exchange} ${marketType} 交易所信息`,
            );
            return { symbols: [] };
        } catch (error) {
            UtilRecord.error(
                `[精度管理] 获取 ${exchange} ${marketType} 交易所信息过程出错:`,
                error?.message || error,
            );
            return { symbols: [] };
        }
    ...
    async getFromDatabase(exchange, marketType) {
        try {
            ...
            try {
                const exchangeInfo =
                    typeof record.exchange_info === "string"
    ...
    async saveToDatabase(exchange, marketType, exchangeInfo) {
        try {
            ...
            UtilRecord.log(
                `[精度管理] ${exchange} ${marketType} 交易所信息已保存到数据库`,
            );
        } catch (error) {
            UtilRecord.error(
                `[精度管理] 保存交易所信息到数据库失败:`,
    ...
    async fetchFromAPI(exchange, marketType, apiKey, apiSecret) {
        for (let attempt = 1; attempt <= this.maxRetries; attempt++) {
            try {
                if (attempt > 1) {
                    UtilRecord.log(
                        `[精度管理] 第 ${attempt} 次尝试从 ${exchange} API获取 ${marketType} 交易所信息...`,
                    );
                ...
                if (exchange === "binance") {
                    exchangeInfo = await this.fetchFromBinance(
                        marketType,
                        apiKey,
                        apiSecret,
                    );
                ...
                UtilRecord.log(
                    `[精度管理] 成功从 ${exchange} API获取 ${marketType} 交易所信息(${exchangeInfo.symbols.length}个交易对)`,
                );
                return exchangeInfo;
            } catch (error) {
                if (attempt === this.maxRetries) {
                    UtilRecord.error(
                        `[精度管理] 从 ${exchange} API获取 ${marketType} 交易所信息失败(已重试${this.maxRetries}次):`,
                        error?.message || error,
                    );
                    throw error;
                }
                UtilRecord.log(
                    `[精度管理] 从 ${exchange} API获取 ${marketType} 交易所信息失败(第${attempt}次尝试):`,
                    error?.message || error,
                );
            }
        }
    ...
    updateInBackground(exchange, marketType, apiKey, apiSecret) {
        setTimeout(async () => {
            try {
                UtilRecord.log(
                    `[精度管理] 开始后台更新 ${exchange} ${marketType} 交易所信息`,
                );
                const exchangeInfo = await this.fetchFromAPI(
                    exchange,
                    marketType,
                ...
                if (
                    ...
                    UtilRecord.log(
                        `[精度管理] 后台更新 ${exchange} ${marketType} 交易所信息完成`,
                    );
                }
            } catch (error) {
                UtilRecord.log(
                    `[精度管理] 后台更新 ${exchange} ${marketType} 交易所信息失败:`,
                    error?.message || error,
                );
            }
        }, 5000); // 延迟5秒执行,避免影响主流程
    ...
    async adjustQuantity(options) {
        ...
        try {
            ...
            return this.smartAdjustQuantity(exchangeInfo, symbol, quantity, {
                silent,
                operationType,
            });
        } catch (error) {
            UtilRecord.error(
                `[精度管理] 调整 ${symbol} 数量精度失败:`,
                error?.message || error,
            );
            // 回退到默认的精度处理
    ...
    smartAdjustQuantity(exchangeInfo, symbol, quantity, options = {}) {
        ...
        if (!silent) {
            ...
            const precision = lotSizeFilter
                ? this.getDecimalPlaces(lotSizeFilter.stepSize)
                : 8;
        ...
        // 根据过滤器调整数量
        const adjustedQuantity = this.adjustQuantityByFilters(
            symbol,
            quantity,
            filters,
            silent,
    ...
    adjustQuantityByFilters(symbol, quantity, symbolFilters, silent = false) {
        ...
        if (!lotSizeFilter) {
            if (!silent)
                UtilRecord.log(
                    `[精度处理] ${symbol} 缺少LOT_SIZE过滤器，使用默认精度`,
                );
            return new bigNumber(quantity).toFixed(8);
        }
    ...
    getQuantityPrecision(exchangeInfo, symbol) {
        ...
        if (!symbolInfo) {
            UtilRecord.log(
                `[精度管理] 未找到交易对 ${symbol} 的信息，使用默认精度 8`,
            );
            return 8;
        }
    ...
    clearMemoryCache(exchange, marketType) {
        if (!exchange) {
            ...
            if (this.memoryCache[exchange]) {
                ...
                UtilRecord.log(
                    `[精度管理] 已清除 ${exchange} ${marketType} 的内存缓存`,
                );
            }
        }
    }
