...
const responseFormatMiddleware = (req, res, next) => {
    ...
    res.on("finish", () => {
        ...
        const elapsedTimeInMs = parseFloat(
            (elapsed[0] * 1000 + elapsed[1] / 1000000).toFixed(2),
        );
        const ip = (req.ip || "").replace(/^::ffff:/, "");
        console.log(
            `[${getCurrentDateTime()}] ${ip} [${req.method}] ${req.originalUrl} - ${res.statusCode} - ${elapsedTimeInMs}ms`,
        );
    });
