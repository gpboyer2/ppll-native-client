...
const getGridPlugin = (trading_mode) => {
    switch (trading_mode) {
        ...
        default:
            throw new Error(
                `不支持的 trading_mode: ${trading_mode}，仅支持 'spot'、'usdt_futures' 和 'coin_futures'`,
            );
    }
};
...
const cleanupSubscriber = async (symbol, id, remark) => {
    try {
        ...
        await GridStrategy.update({ remark }, { where: { id } });
    } catch (e) {
...
async function findOrCreateStrategy(params) {
    ...
    const { api_key, api_secret, trading_pair, position_side, trading_mode } =
        params.original;
    ...
    try {
        ...
        console.error(
            "[grid-strategy.service] InfiniteGrid.create 失败:",
            error,
        );
...
async function bindEventHandlers(row, params) {
    ...
    instance.onWarn = async function (data) {
        ...
        await usd_m_futures_infinite_grid_event_manager.logEvent({
            ...
            eventType:
                usd_m_futures_infinite_grid_event_manager.eventTypes.WARN,
            level: "warn",
    ...
    instance.onOpenPosition = async function (data) {
        const gridTradeQuantity =
            instance.config.position_side === "LONG"
                ? instance.config.grid_long_open_quantity ||
                  instance.config.grid_trade_quantity
                : instance.config.grid_short_open_quantity ||
                  instance.config.grid_trade_quantity;
        ...
        await createTradeHistory({
            ...
            max_position_quantity:
                instance.config.max_open_position_quantity || 0,
            min_position_quantity:
                instance.config.min_open_position_quantity || 0,
            fall_prevention_coefficient:
                instance.config.fall_prevention_coefficient || 0,
            ...
            console.error(
                "Error creating trade history for open position:",
                err,
            );
        });
        ...
        await usd_m_futures_infinite_grid_event_manager.logEvent({
            ...
            eventType:
                usd_m_futures_infinite_grid_event_manager.eventTypes
                    .OPEN_POSITION,
            level: "success",
    ...
    instance.onClosePosition = async function (data) {
        const gridTradeQuantity =
            instance.config.position_side === "LONG"
                ? instance.config.grid_long_close_quantity ||
                  instance.config.grid_trade_quantity
                : instance.config.grid_short_close_quantity ||
                  instance.config.grid_trade_quantity;
        ...
        await createTradeHistory({
            ...
            console.error(
                "Error creating trade history for close position:",
                err,
            );
        });
...
const createGridStrategy = async (
    /** @type {{api_key: string, api_secret: string, trading_pair: string, position_side: string, trading_mode?: string}} */ params,
) => {
    // 步骤1: 验证和清洗参数
            ...
            if (running_instance) {
                ...
                return {
                    ...
                    total_open_position_quantity:
                        running_instance.total_open_position_quantity ??
                        row_data.total_open_position_quantity,
                    total_open_position_value:
                        running_instance.total_open_position_value ??
                        row_data.total_open_position_value,
                    total_open_position_entry_price:
                        running_instance.total_open_position_entry_price ??
                        row_data.total_open_position_entry_price,
                    break_even_price:
                        running_instance.break_even_price ??
                        row_data.break_even_price,
                };
            }
...
const updateGridStrategyById = async (updateBody) => {
    ...
    // 获取当前策略数据，用于状态校验
    const currentStrategy = await GridStrategy.findOne({
    ...
    if (
        ...
        (currentStrategy.status === "STOPPED" ||
            currentStrategy.status === "DELETED")
    ) {
        throw new Error(
            `无法${paused ? "暂停" : "恢复"}策略：策略状态为 ${currentStrategy.status}`,
        );
    }
    ...
    if (affectedCount > 0) {
        ...
        if (paused === false) {
            if (gridMap[id]) {
                ...
                UtilRecord.log("[grid-strategy] 策略实例重新创建成功", {
                    strategyId: id,
                });
            }
        }
    ...
    for (const strategy_id of id_list) {
        if (gridMap[strategy_id]) {
            try {
                ...
                console.error(
                    `[grid-strategy] 清理策略 ${strategy_id} 时出错:`,
                    e,
                );
                // 忽略清理策略时的错误，继续执行删除逻辑
            }
    ...
    for (const strategy of strategies) {
        ...
        if (symbol) {
            ...
            if (subs) {
                ...
                UtilRecord.log("[grid-strategy] 清理后订阅者数量", {
                    symbol,
                    remaining: subs.size,
                });
                if (subs.size === 0) {
                    gridStrategyRegistry.delete(symbol);
                    global.wsManager.unsubscribeMarkPrice(symbol);
                    UtilRecord.log("[grid-strategy] 已取消订阅", {
                        symbol,
                        strategyId: strategy_id,
                    });
                }
                ...
                UtilRecord.log(
                    "[grid-strategy] registry 中无记录，强制取消订阅",
                    { symbol, strategyId: strategy_id },
                );
                global.wsManager.unsubscribeMarkPrice(symbol);
            }
        }
