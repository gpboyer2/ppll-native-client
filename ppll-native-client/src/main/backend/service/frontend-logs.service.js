...
async function cleanup_old_logs() {
    ...
    if (now - last_cleanup_time < CLEANUP_INTERVAL) {
        UtilRecord.trace(
            "[FrontendLogService] 距离上次清理不足间隔时间，跳过本次清理",
        );
        return;
    }
    ...
    try {
        ...
        if (deleted_count > 0) {
            UtilRecord.trace(
                `[FrontendLogService] 清理 ${deleted_count} 条旧日志（${RETENTION_MINUTES}分钟前）`,
            );
        }
    } catch (error) {
        UtilRecord.trace(
            `[FrontendLogService] 清理旧日志失败: ${error.message}`,
        );
    }
}
    ...
    try {
        ...
        const log_promises = log_list.map((log) =>
            FrontendLog.create({
                ...
                // 静默失败，避免日志系统本身产生大量错误
                UtilRecord.trace(
                    `[FrontendLogService] 保存前端日志失败: ${err.message}`,
                );
                return null;
            }),
        );
        ...
        UtilRecord.trace(
            `[FrontendLogService] 保存前端日志失败: ${error.message}`,
        );
        return [];
    }
}
