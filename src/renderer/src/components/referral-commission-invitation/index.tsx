...
interface CommissionCalculationData {
  trade_value: number // 每笔交易金额
  daily_frequency: number // 日交易次数
  commission_rate: string // 手续费率
  rebate_rate: string // 返佣比例
  daily_rebate_profit: number // 日返佣收益
  monthly_extra_profit: number // 月额外收益
  current_monthly_profit: number // 当前月收益（无返佣）
  rebate_monthly_profit: number // 开启返佣后月收益
  current_yearly_profit: number // 当前年收益（无返佣）
  rebate_yearly_profit: number // 开启返佣后年收益
  extra_profit_rate: number // 额外收益百分比
}
...
export function ReferralCommissionDialog({
  ...
  const calculatedData = useMemo<CommissionCalculationData | null>(() => {
    ...
    if (expected_daily_frequency && expected_daily_profit && trade_value) {
      ...
      const monthlyTradingFee =
        expected_daily_frequency * daysInMonth * trade_value * commissionRate
      ...
      return {
        ...
        current_yearly_profit: parseFloat(
          NumberFormat.formatAmount(expected_daily_profit * daysInYear)
        ),
        rebate_yearly_profit: parseFloat(
          NumberFormat.formatAmount((expected_daily_profit + dailyRebate) * daysInYear)
        ),
    ...
    // 网格差价越小，触发频次越高
    const estimatedDailyFrequency = Math.max(
      3,
      Math.min(20, Math.floor(100 / grid_price_difference))
    )

    // 估算日收益：每次网格交易的利润 × 日频次
    // 每次网格利润 ≈ 网格差价 × 交易数量 × 0.5（考虑双向交易）
    const estimatedDailyProfit =
      grid_price_difference * tradeQuantity * estimatedDailyFrequency * 0.5
    ...
    const monthlyTradingFee =
      estimatedDailyFrequency * daysInMonth * estimatedTradeValue * commissionRate
  ...
  const data = calculatedData || {
    ...
    commission_rate: '1‰',
    rebate_rate: '最高 35%',
    daily_rebate_profit: 5,
    monthly_extra_profit: 150,
    current_monthly_profit: 465,
    rebate_monthly_profit: 615,
    current_yearly_profit: 5580,
    rebate_yearly_profit: 7380,
    extra_profit_rate: 32
  }
          ...
          <div className="referral-commission-dialog-details-section">
              ...
              <table>
                ...
                <tbody>
                  ...
                  <tr className="referral-commission-dialog-table-total">
                    ...
                    <td>
                      <strong>{NumberFormat.formatAmount(data.current_yearly_profit)} USDT</strong>
                    </td>
                    <td className="referral-commission-dialog-table-highlight-text">
                      <strong>
                        {NumberFormat.formatAmount(
                          data.rebate_yearly_profit - data.current_yearly_profit
                        )}{' '}
                        USDT
                      </strong>
                    </td>
            ...
            <div className="referral-commission-dialog-params-info">
              <div className="referral-commission-dialog-param-item">
                ...
                <span className="referral-commission-dialog-param-value">
                  {NumberFormat.formatAmount(data.trade_value)} USDT
                </span>
              </div>
              <div className="referral-commission-dialog-param-item">
                <span className="referral-commission-dialog-param-label">日交易次数：</span>
                <span className="referral-commission-dialog-param-value">
                  {NumberFormat.truncateDecimal(data.daily_frequency)} 次
                </span>
              </div>
              <div className="referral-commission-dialog-param-item">
