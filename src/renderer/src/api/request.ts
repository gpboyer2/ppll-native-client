...
class RequestLogger {
  ...
  static logError(requestId: number, error: any, startTime: number): void {
    ...
    console.error(`[错误 #${requestId}]`, `耗时: ${duration}ms`, '\n错误:', error)
  }
...
export class RequestWrapper {
  ...
  private static needsApiKey(url: string): boolean {
    return (
      this.shouldUseNodejs(url) &&
      !this.NO_API_KEY_URLS.some((noApiKeyUrl) => url.startsWith(noApiKeyUrl))
    )
  }
  ...
  static async get<T = any>(url: string, params?: Record<string, any>): Promise<Response<T>> {
    ...
    const mergedParams = this.needsApiKey(url) ? { ...autoParams, ...params } : params
  ...
  static async post<T = any>(url: string, data?: any): Promise<Response<T>> {
    ...
    const mergedData = this.needsApiKey(url) ? { ...this.getApiKeyParams(), ...data } : data
    return this.request<T>(
      'POST',
      url,
      () => apiClient.post<T>(url, mergedData),
      undefined,
      mergedData
    )
  }
  ...
  static async put<T = any>(url: string, data?: any): Promise<Response<T>> {
    ...
    return this.request<T>(
      'PUT',
      url,
      () => apiClient.put<T>(url, mergedData),
      undefined,
      mergedData
    )
  }
  ...
  static async delete<T = any>(url: string, data?: any): Promise<Response<T>> {
    ...
    return this.request<T>(
      'DELETE',
      url,
      () => apiClient.delete<T>(url, { data: mergedData }),
      undefined,
      mergedData
    )
  }
  ...
  static async patch<T = any>(url: string, data?: any): Promise<Response<T>> {
    ...
    return this.request<T>(
      'PATCH',
      url,
      () => apiClient.patch<T>(url, mergedData),
      undefined,
      mergedData
    )
  }
  ...
  static async upload<T = any>(url: string, file: File): Promise<Response<T>> {
    const requestId = RequestLogger.logStart('UPLOAD', url, undefined, {
      fileName: file.name,
      size: file.size
    })
    const startTime = Date.now()
