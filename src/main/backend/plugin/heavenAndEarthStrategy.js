...
function HeavenAndEarthStrategy(options) {
    ...
    this.resetTargetPrice = (executionPrice) => {
        ...
        this.nextExpectedRisePrice = bigNumber(executionPrice)
            .plus(this.config.grid_price_difference)
            .toNumber();
        this.nextExpectedFallPrice = bigNumber(executionPrice)
    ...
    this.queryOrder = async (orderId, prePositionQty, orderQty, orderType) => {
        ...
        const isSuccess =
            Math.abs(this.totalOpenPositionQuantity - expectedQty) <=
            bigNumber(orderQty).times(0.001).toNumber();
        this.logger.log(
            `📊 持仓推断: 订单前=${prePositionQty}, 预期=${expectedQty}, 当前=${this.totalOpenPositionQuantity}, 推断${isSuccess ? "成功" : "失败"}`,
        );
        return isSuccess
            ? {
                  orderId,
    ...
    this.createMultipleOrders = async (positionQuantity) => {
        ...
        this.totalOpenPositionQuantity = bigNumber(
            this.totalOpenPositionQuantity,
        )
            .plus(positionQuantity)
            .toNumber();
    ...
    this.getAccountInfo = async () => {
        let errorMsg = null;
        let accountInfo = await this.client
    ...
    this.initAccountInfo = async () => {
        ...
        try {
            ...
            let tradingPairPosition = accountInfo.positions.find(
                (item) =>
                    item.symbol === this.config.tradingPair &&
                    item.positionSide === "LONG",
            );
            this.totalOpenPositionQuantity = Number(
                tradingPairPosition.positionAmt,
            );
            this.totalOpenPositionEntryPrice =
                Number(tradingPairPosition.entryPrice) <
                this.totalOpenPositionEntryPrice
                    ? this.totalOpenPositionEntryPrice
                    : Number(tradingPairPosition.entryPrice);
        } catch (error) {
    ...
    this.heavenAndEarthWebsocket = async ({ latestPrice }) => {
        ...
        if (this.config.pollingInterval)
            this.throttleEnabled = setTimeout(
                () => (this.throttleEnabled = false),
        ...
        this.logger.log(
            `当前总持仓数量为 ${this.totalOpenPositionQuantity}/${this.config.tradingPair}, 限制最大持仓数量为 ${this.config.maxOpenPositionQuantity}/${this.config.tradingPair}`,
        );
        this.logger.log(
            `期望下次涨至某价格: ${this.nextExpectedRisePrice}, 期望下次跌至某价格: ${this.nextExpectedFallPrice}`,
        );
        this.logger.log(
            `累计已成交 ${this.logs.length} 次，其中开仓多单 ${this.logs.filter((p) => p.side === "BUY").length} 次，平仓多单 ${this.logs.filter((p) => p.side === "SELL").length} 次`,
        );
        this.logger.log(
            `当前持仓平均开仓价为 ${this.totalOpenPositionEntryPrice}`,
        );
    ...
    this.initOrders = async () => {
        ...
        if (this.totalOpenPositionQuantity < minOpenPositionQuantity) {
            let quantity = bigNumber(minOpenPositionQuantity)
                ...
                .toNumber();
            await this.createMultipleOrders(quantity).catch((err) =>
                this.logger.error("初始化开仓失败", err),
            );
        }
