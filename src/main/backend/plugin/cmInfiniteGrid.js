/**
 * 无限网格策略（B本位合约版本）
 * 基于 umInfiniteGrid.js 适配B本位合约交易
 * 支持做多和做空持仓方向
 */
 ...
 * @param {string} options.trading_pair - 交易对，例如`BTCUSD_PERP`
...
function InfiniteGrid(options) {
    ...
    let coinmClientConfig = {};
    if (process.env.NODE_ENV !== "production") {
        const proxyConfig = getProxyConfig();
        if (proxyConfig) {
            coinmClientConfig.proxy = proxyConfig;
        }
    }

    /** 调用binance生成的客户端（B本位合约） */
    ...
    this.getAccountInfo = async () => {
        try {
            ...
            // 通过 api_key 实现数据隔离
            const accountInfo =
                await binanceAccountService.getCoinMFuturesAccount(
                    this.config.api_key,
    ...
    this._initOrders = async () => {
        ...
        let { min_open_position_quantity, max_open_position_quantity } =
            this.config;
