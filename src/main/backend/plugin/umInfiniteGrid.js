...
const {
    createTradeHistory,
} = require("../service/grid-trade-history.service.js");
...
function InfiniteGrid(options) {
    ...
    // 检查做空方向的分离数量配置
    const has_short_quantities =
        options.grid_short_open_quantity &&
        options.grid_short_open_quantity > 0 &&
        options.grid_short_close_quantity &&
        options.grid_short_close_quantity > 0;

    // 根据持仓方向检查数量配置
    if (options.position_side === "LONG") {
        if (!has_grid_trade_quantity && !has_long_quantities) {
            UtilRecord.log(
                `❗️ 做多方向必须配置 'grid_trade_quantity' 或者同时配置 'grid_long_open_quantity' 和 'grid_long_close_quantity'，且值必须大于0`,
            );
            return;
        }
    }
    if (options.position_side === "SHORT") {
        if (!has_grid_trade_quantity && !has_short_quantities) {
            UtilRecord.log(
                `❗️ 做空方向必须配置 'grid_trade_quantity' 或者同时配置 'grid_short_open_quantity' 和 'grid_short_close_quantity'，且值必须大于0`,
            );
            return;
        }
    }
    ...
    if (
        !this.config.position_side ||
        (this.config.position_side !== "LONG" &&
            this.config.position_side !== "SHORT")
    ) {
        UtilRecord.log(
            `❗️ 必填项'position_side'不能为空，且必须为'LONG'或'SHORT'`,
        );
        return;
    }
    ...
    this.handleCloseOrderError = (error) => {
        ...
        switch (errorCode) {
            ...
            case -2022:
                this.logger.warn(
                    `检测到仓位已被手动平仓（错误码-2022），清空开仓历史记录并重新初始化情况`,
                );
                this.position_open_history = [];
                this.total_open_position_quantity = 0;
    ...
    this.getShortOpenQuantity = () => {
        return (
            this.config.grid_short_open_quantity ||
            this.config.grid_trade_quantity
        );
    };
    ...
    this.getShortCloseQuantity = () => {
        return (
            this.config.grid_short_close_quantity ||
            this.config.grid_trade_quantity
        );
    };
        ...
        try {
            ...
            const tradeData = {
                ...
                max_position_quantity:
                    this.config.max_open_position_quantity || 0,
                min_position_quantity:
                    this.config.min_open_position_quantity || 0,
                fall_prevention_coefficient:
                    this.config.fall_prevention_coefficient || 0,
                entry_order_id:
                    type === "open" ? orderDetail.orderId || "" : "",
                exit_order_id:
                    type === "close" ? orderDetail.orderId || "" : "",
                grid_level: 0,
                entry_price: type === "open" ? orderDetail.avgPrice || 0 : 0,
                exit_price: type === "close" ? orderDetail.avgPrice || 0 : 0,
                position_quantity:
                    orderDetail.executedQty || orderDetail.cumQty || 0,
                profit_loss: 0,
                ...
                entry_time:
                    type === "open"
                        ? new Date(orderDetail.time || Date.now())
                        : null,
                exit_time:
                    type === "close"
                        ? new Date(orderDetail.time || Date.now())
                ...
                avg_entry_price:
                    type === "open" ? orderDetail.avgPrice || 0 : 0,
                avg_exit_price:
                    type === "close" ? orderDetail.avgPrice || 0 : 0,
                price_difference: 0,
            ...
            this.logger.debug(
                `[数据库写入成功] ${type === "open" ? "建仓" : "平仓"}交易历史已写入, orderId=${orderDetail.orderId}`,
            );
        } catch (error) {
            this.logger.error(
                `[数据库写入失败] ${type === "open" ? "建仓" : "平仓"}交易历史写入失败:`,
                error.message,
            );
        }
    };
    ...
    this.resetTargetPrice = (executionPrice) => {
        if (!executionPrice || !this.config.grid_price_difference) {
            this.logger.warn(
                `重置期望价格失败，executionPrice: ${executionPrice}, grid_price_difference: ${this.config.grid_price_difference}`,
            );
            return;
        }
        ...
        if (this.config.position_side === "LONG") {
            // 做多逻辑
            this.next_expected_rise_price = bigNumber(executionPrice)
                .plus(this.config.grid_price_difference)
                .toNumber();
            let coefficient = bigNumber(this.config.grid_price_difference)
                .times(
                    bigNumber(this.total_open_position_quantity).div(
            ...
            this.next_expected_fall_price = bigNumber(executionPrice)
    ...
    this.hasPendingOrder = () => {
        if (
            this.position_open_history &&
            this.position_open_history.length > 0
        ) {
            const last =
                this.position_open_history[
                    this.position_open_history.length - 1
                ];
    ...
    this.queryOrder = async (orderId, prePositionQty, orderQty, orderType) => {
        ...
        for (let i = 0; i <= MAX_RETRY; i++) {
            ...
            try {
                ...
                this.logger.sql(GridEventTypes.ORDER, `查询订单详情`, {
                    order: res,
                });
        ...
        const isSuccess =
            Math.abs(this.total_open_position_quantity - expectedQty) <=
            bigNumber(orderQty).times(0.001).toNumber();
        this.logger.log(
            `📊 持仓推断: 订单前=${prePositionQty}, 预期=${expectedQty}, 当前=${this.total_open_position_quantity}, 推断${isSuccess ? "成功" : "失败"}`,
        );
        ...
        return isSuccess
            ? {
                  orderId,
                  avgPrice: String(this.latestPrice || 0),
                  status: "INFERRED",
              }
            : null;
    };
    ...
    this.openOrders = async (positionQuantity) => {
        ...
        try {
            ...
            this.logger.sql(GridEventTypes.ORDER, `创建仓位订单`, {
                order: res,
            });
            ...
            this.logger
                .sql(
                    GridEventTypes.ERROR,
                    `创建${this.config.position_side === "LONG" ? "多" : "空"}单仓位失败: ${error.message}`,
                    { error: error.message },
                )
                .error();
            if (typeof this.onWarn === "function")
        ...
        if (!orderDetail) {
            this.logger.warn(
                `创建${this.config.position_side === "LONG" ? "多" : "空"}单后，无法查询订单详情`,
            );
            // 移除占位符
            this.position_open_history.pop();
            this.order_options.lock = "idle";
    ...
    this.closeOrders = async (positionQuantity) => {
        ...
        try {
            ...
            this.logger
                .sql(
                    GridEventTypes.ERROR,
                    `平${this.config.position_side === "LONG" ? "多" : "空"}单仓位失败: ${error.message}`,
                    { error: error.message },
                )
                .error();
            if (typeof this.onWarn === "function")
        ...
        if (!orderDetail) {
            this.logger.warn(
                `平掉${this.config.position_side === "LONG" ? "多" : "空"}单后，无法查询订单详情`,
            );
            this.order_options.lock = "idle";
            return;
        }
    ...
    this.getAccountInfo = async () => {
        try {
            ...
            // 通过 api_key 实现数据隔离
            const accountInfo =
                await binanceAccountService.getUSDMFuturesAccount(
                    this.config.api_key,
    ...
    this.initAccountInfo = async (shouldThrow = false) => {
        let accountInfo = await this.getAccountInfo().catch((error) => {
            ...
            if (error?.code === -2014 || error?.code === -2015) {
                this.updateExecutionStatus(
                    execution_status.API_KEY_INVALID,
                ).catch(() => {});
            }
        ...
        try {
            ...
            this.trading_pair_info = accountInfo.positions.find(
                (item) =>
                    item.symbol === this.config.trading_pair &&
                    item.positionSide === this.config.position_side,
            );

            // 做多
            if (this.config.position_side === "LONG") {
                this.total_open_position_quantity = Number(
                    this.trading_pair_info.positionAmt,
                );
                this.total_open_position_entry_price =
                    Number(this.trading_pair_info.entryPrice) ||
                    this.total_open_position_entry_price;
                this.break_even_price =
                    Number(this.trading_pair_info.breakEvenPrice) || 0;
            }
            // 做空
            else {
                this.total_open_position_quantity = Math.abs(
                    Number(this.trading_pair_info.positionAmt),
                );
                this.total_open_position_entry_price =
                    Number(this.trading_pair_info.entryPrice) ||
                    this.total_open_position_entry_price;
            ...
            this.logger
                .sql(
                    GridEventTypes.ERROR,
                    `accountInfo 数据异常: ${error.message}`,
                    { error: error.message },
                )
                .error();
    ...
    this.gridWebsocket = async ({ latestPrice }) => {
        ...
        if (
            ...
            this.logger
                .sql(
                    GridEventTypes.GRID,
                    `⛔️ 币价${latestPrice}小于等于限制价格${lt_limitation_price}，自动暂停网格`,
                )
                .log();
            ...
            this.logger
                .sql(
                    GridEventTypes.GRID,
                    `⛔️ 币价${latestPrice}大于等于限制价格${gt_limitation_price}，自动暂停网格`,
                )
                .log();
        ...
        if (
            latestPrice >= this.trading_pair_info.entryPrice &&
            this.config.is_above_open_price
        ) {
            this.logger
                .sql(
                    GridEventTypes.GRID,
                    `⛔️ 币价${latestPrice}大于等于开仓价格${this.trading_pair_info.entryPrice}，自动暂停网格`,
                )
                .log();
            await this.onPausedGrid(execution_status.PRICE_ABOVE_OPEN);
        } else if (
            latestPrice <= this.trading_pair_info.entryPrice &&
            this.config.is_below_open_price
        ) {
            this.logger
                .sql(
                    GridEventTypes.GRID,
                    `⛔️ 币价${latestPrice}小于等于开仓价格${this.trading_pair_info.entryPrice}，自动暂停网格`,
                )
                .log();
        ...
        if (this.config.polling_interval)
            this.throttle_enabled = setTimeout(
                () => (this.throttle_enabled = false),
        ...
        //  - 初始化账户信息与仓位信息；
        if (
            !this.total_open_position_quantity ||
            !this.position_open_history?.length
        ) {
            this.logger.warn(
                `当前已没有仓位信息，重新初始化账户信息与仓位信息用以同步最新数据`,
            );
            this.logger.debug(
        ...
        if (
            this.count % 100 === 0 ||
            !this.lastAccountInfoUpdate ||
            Date.now() - this.lastAccountInfoUpdate > 300000
        ) {
            this.logger
                .sql(
                    GridEventTypes.ACCOUNT,
                    `🔄 定期刷新账户信息以同步最新余额 (轮询第${this.count}次)`,
                )
                .log();
            await this.initAccountInfo().catch(() => {});
        }
        ...
        this.logger.log(
            `💰 ${this.config.position_side === "LONG" ? "做多" : "做空"}网格策略(ID: ${this.config.id}). 轮询第 ${this.count} 次`,
        );
        this.count += 1;
        ...
        this.logger.log(
            `当前总持仓数量为 ${this.total_open_position_quantity}/${this.config.trading_pair}, 限制最大持仓数量为 ${this.config.max_open_position_quantity}/${this.config.trading_pair}`,
        );

        if (this.config.position_side === "LONG")
            this.logger.log(
                `每次增加多单数量: ${longOpenQuantity}, 每次减少多单数量: ${longCloseQuantity}, 网格之间的价格差价: ${this.config.grid_price_difference}`,
            );
        if (this.config.position_side === "SHORT")
            this.logger.log(
                `每次增加空单数量: ${shortOpenQuantity}, 每次减少空单数量: ${shortCloseQuantity}, 网格之间的价格差价: ${this.config.grid_price_difference}`,
            );
        ...
        this.logger.log(
            `下次网格减仓时匹配的利润预计为(扣除0.1%手续费): ${this.getGridProfit(latestPrice)}, `,
        );

        this.logger.log(
            `允许顺势仅减仓策略: ${this.config.priority_close_on_trend}`,
        );
        this.logger.log(
            `期望下次涨至某价格:`,
        ...
        if (this.config.position_side === "LONG") {
            this.logger.log(
                `累计已成交 ${this.logs.length} 次，其中开仓多单 ${this.logs.filter((p) => p.side === "BUY").length} 次，平仓多单 ${this.logs.filter((p) => p.side === "SELL").length} 次`,
            );
        } else {
            this.logger.log(
                `累计已成交 ${this.logs.length} 次，其中开仓空单 ${this.logs.filter((p) => p.side === "SELL").length} 次，平仓空单 ${this.logs.filter((p) => p.side === "BUY").length} 次`,
            );
        }

        this.logger.log(
            `API返回的平均开仓价: ${this.total_open_position_entry_price}, 保本价: ${this.break_even_price}`,
        );
        ...
        if (
            ...
            if (
                ...
                this.logger
                    .sql(
                        GridEventTypes.GRID,
                        `🔄 启用顺势仅减仓策略：当前实际仓位数量为 ${this.total_open_position_quantity}/${this.config.trading_pair}， 足够平仓，且当前仍处于${this.config.position_side === "LONG" ? "上涨" : "下跌"}趋势，因此跳过创建新仓位`,
                    )
                    .log();
            } else {
                this.logger
                    .sql(
                        GridEventTypes.GRID,
                        `😎 缓存中没有${this.config.position_side === "LONG" ? "多" : "空"}单仓位且没有超过最大持仓数量限制, 增加一个新的${this.config.position_side === "LONG" ? "多" : "空"}单仓位`,
                    )
                    .log();
        ...
        if (this.config.position_side === "LONG") {
            ...
            if (
                ...
                this.logger
                    .sql(
                        GridEventTypes.GRID,
                        `⬆️ 币价${latestPrice}上涨，匹配上一个网格价格${lastPosition?.avgPrice}平仓`,
                    )
                    .log();
        ...
        // 做空逻辑
            ...
            if (
                ...
                this.logger
                    .sql(
                        GridEventTypes.GRID,
                        `⬇️ 币价${latestPrice}下跌，匹配上一个空单网格盈利价${lastPosition?.avgPrice}平仓`,
                    )
                    .log();
        ...
        if (
            ...
            this.logger
                .sql(
                    GridEventTypes.GRID,
                    `⛔️ 持仓数量${this.total_open_position_quantity}已达上限${this.config.max_open_position_quantity}，停止加仓`,
                )
                .log();
            return;
        }
        ...
        if (
            ...
            this.logger
                .sql(
                    GridEventTypes.GRID,
                    `😎 持仓数量${this.total_open_position_quantity}低于最小值${this.config.min_open_position_quantity}，立即加仓`,
                )
                .log();
    ...
    this._initOrders = async () => {
        // 设置状态为初始化中（注意：记录创建时已经设置为 INITIALIZING，这里不再重复更新）
        this.onPausedGrid();
        ...
        try {
            ...
            const errorMsg =
                "初始化账户信息失败，请检查 API Key 配置（可能存在 IP 白名单限制）";
            this.logger
                .sql(GridEventTypes.ERROR, `${errorMsg}: ${error.message}`, {
                    error: error.message,
                })
                .error();
        ...
        if (
            ...
            this.total_open_position_quantity < min_open_position_quantity
        ) {
        ...
        this.onContinueGrid();
        this.logger
            .sql(GridEventTypes.INIT, `✅ 策略初始化完成，网格已恢复运行`)
            .log();
