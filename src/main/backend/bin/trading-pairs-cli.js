...
program
    ...
    .action(async (options) => {
        try {
            ...
            const result =
                await tradingPairsComparisonService.getFuturesOnlyPairs();
            ...
            const displayPairs =
                limit > 0 ? result.pairs.slice(0, limit) : result.pairs;

            console.log(`📋 交易对列表 (显示前 ${displayPairs.length} 个):`);
            displayPairs.forEach((pair, index) => {
                console.log(
                    `   ${(index + 1).toString().padStart(3)}: ${pair}`,
                );
            });

            if (result.pairs.length > limit) {
                console.log(
                    `   ... 还有 ${result.pairs.length - limit} 个交易对`,
                );
            }
        } catch (error) {
...
program
    ...
    .action(async (options) => {
        try {
            ...
            const result =
                await tradingPairsComparisonService.getSpotOnlyPairs();
...
program
    ...
    .action(async (options) => {
        try {
            ...
            const result =
                await tradingPairsComparisonService.getComparisonReport();

            console.log(`📊 总体统计:`);
            console.log(
                `   总现货交易对数: ${result.summary.total_spot_pairs}`,
            );
            console.log(
                `   总合约交易对数: ${result.summary.total_futures_pairs}`,
            );
            console.log(`   共同交易对数: ${result.summary.common_pairs}`);
            console.log(
                `   仅合约交易对数: ${result.summary.futures_only_count}`,
            );
...
program
    ...
    .action(async (symbol) => {
        try {
            ...
            const result =
                await tradingPairsComparisonService.analyzeTradingPairAvailability(
                    symbol.toUpperCase(),
                );
...
program
    ...
    .action(async (options) => {
        try {
            ...
            const result =
                await tradingPairsComparisonService.getBaseAssetAnalysis();

            console.log(`📊 基础资产统计:`);
            console.log(`   总基础资产数: ${result.summary.total_base_assets}`);
            console.log(`   共同资产数: ${result.summary.common_assets_count}`);
            console.log(
                `   仅现货资产数: ${result.summary.spot_only_assets_count}`,
            );
            console.log(
                `   仅合约资产数: ${result.summary.futures_only_assets_count}`,
            );
            ...
            if (options.details) {
                if (result.details.futures_only_assets.length > 0) {
                    console.log(
                        `📈 仅合约资产 (${result.details.futures_only_assets.length}个):`,
                    );
                    result.details.futures_only_assets
                        .slice(0, 30)
                        .forEach((asset, index) => {
                            console.log(
                                `   ${(index + 1).toString().padStart(3)}: ${asset}`,
                            );
                        });
                    if (result.details.futures_only_assets.length > 30) {
                        console.log(
                            `   ... 还有 ${result.details.futures_only_assets.length - 30} 个资产`,
                        );
                    }
                    console.log();
                }
                ...
                if (result.details.spot_only_assets.length > 0) {
                    console.log(
                        `💰 仅现货资产 (${result.details.spot_only_assets.length}个):`,
                    );
                    result.details.spot_only_assets
                        .slice(0, 30)
                        .forEach((asset, index) => {
                            console.log(
                                `   ${(index + 1).toString().padStart(3)}: ${asset}`,
                    ...
                    if (result.details.spot_only_assets.length > 30) {
                        console.log(
                            `   ... 还有 ${result.details.spot_only_assets.length - 30} 个资产`,
                        );
                    }
                    console.log();
                }
                ...
                if (result.details.common_assets.length > 30) {
                    console.log(
                        `   ... 还有 ${result.details.common_assets.length - 30} 个资产`,
                    );
                }
            }
        } catch (error) {
...
program
    ...
    .action(async (symbols) => {
        try {
            ...
            console.log(`交易对分析结果:`);
            console.log(
                `${"交易对".padEnd(15)} | ${"现货".padEnd(6)} | ${"合约".padEnd(6)} | 分类`,
            );
            ...
            for (const symbol of symbols) {
                try {
                    ...
                    console.log(
                        `${result.symbol.padEnd(15)} | ${spotStatus.padEnd(6)} | ${futuresStatus.padEnd(6)} | ${result.category}`,
                    );
                } catch (error) {
                    console.log(
                        `${symbol.toUpperCase().padEnd(15)} | ${"❌".padEnd(6)} | ${"❌".padEnd(6)} | 分析失败`,
                    );
                }
            }
        } catch (error) {
