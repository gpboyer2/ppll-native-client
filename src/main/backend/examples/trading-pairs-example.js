...
async function basicExample() {
    ...
    try {
        ...
        for (const symbol of symbols) {
            const analysis =
                await tradingPairsService.analyzeTradingPairAvailability(
                    symbol,
                );
...
async function advancedExample() {
    ...
    try {
        ...
        console.log("   基础资产统计:");
        console.log(
            `   - 总基础资产数: ${assetAnalysis.summary.total_base_assets}`,
        );
        console.log(
            `   - 共同资产数: ${assetAnalysis.summary.common_assets_count}`,
        );
        console.log(
            `   - 仅现货资产数: ${assetAnalysis.summary.spot_only_assets_count}`,
        );
        console.log(
            `   - 仅合约资产数: ${assetAnalysis.summary.futures_only_assets_count}`,
        );

        if (assetAnalysis.details.futures_only_assets.length > 0) {
            console.log(
                `   - 仅合约资产示例: ${assetAnalysis.details.futures_only_assets.slice(0, 10).join(", ")}`,
            );
        }
        console.log();
    } catch (error) {
...
async function sdkExample() {
    ...
    try {
        ...
        const activePairs = spotInfo.symbols.filter(
            (s) => s.status === "TRADING",
        );
...
async function utilityExample() {
    ...
    try {
        ...
        const futuresPairs =
            await tradingPairsService.fetchFuturesTradingPairs();
        const thousandPairs = futuresPairs.filter((pair) =>
            pair.includes("1000"),
        );
        ...
        const futuresUsdcPairs = futuresPairs.filter((pair) =>
            pair.endsWith("USDC"),
        );
        ...
        for (const coin of popularCoins) {
            ...
            const status =
                analysis.hasSpot && analysis.hasFutures
                    ? "✅"
                    : analysis.hasSpot
                      ? "📊"
                      : analysis.hasFutures
                        ...
                        : "❌";
            console.log(
                `   ${usdtPair.padEnd(10)} ${status} ${analysis.category}`,
            );
        }
        console.log(
