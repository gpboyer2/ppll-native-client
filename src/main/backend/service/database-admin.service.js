...
const getTableDetail = async (params) => {
    ...
    try {
        ...
        const indexInfo = await db.sequelize.query(
            `PRAGMA index_list("${tableName}")`,
        );
        const indexes = [];
        for (const index of indexInfo[0]) {
            const indexColumns = await db.sequelize.query(
        ...
        const createSql =
            createTable[0].length > 0 ? createTable[0][0].sql : "";
...
const deleteColumn = async (params) => {
    try {
        ...
        await db.sequelize.query(
            `CREATE TABLE "${newTableName}" (${columnDefs})`,
        );
        await db.sequelize.query(
            `INSERT INTO "${newTableName}" SELECT ${columnList} FROM "${tableName}"`,
        );
...
const copyTable = async (params) => {
    try {
        ...
        if (copyData) {
            // 复制表结构和数据
            await db.sequelize.query(
                `CREATE TABLE "${newName}" AS SELECT * FROM "${tableName}"`,
            );
        } else {
            // 仅复制表结构
            const createTable = await db.sequelize.query(
            ...
            const createSql = createTable[0][0].sql.replace(
...
const renameColumn = async (params) => {
    try {
        ...
        // 检查新列名是否已存在
        const newColumnExists = tableInfo[0].some(
...
const createIndex = async (params) => {
    try {
        ...
        await db.sequelize.query(
            `CREATE ${uniqueKeyword} INDEX "${indexName}" ON "${tableName}" (${columnsStr})`,
        );
