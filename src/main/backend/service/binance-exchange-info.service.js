        ...
        return await new Promise((resolve, reject) => {
            request(requestOptions, (error, response, body) => {
                if (error) {
                    ...
                    console.warn(
                        `下架计划API返回错误: HTTP ${response.statusCode}:`,
                        body,
                    );
                    resolve([]); // 失败时返回空数组
                } else {
 ...
 */
const getDelistingPerpetualContracts = (
    exchangeInfo,
    delistSchedule = [],
    daysAhead = 30,
) => {
    ...
    return exchangeInfo.symbols
        .filter((symbol) => {
            ...
            // 检查是否在预警时间范围内且未过期
            const isInAlertRange =
                delistTime <= alertTime && delistTime > currentTime;
            // 确保合约当前处于交易状态
        ...
        .map((symbol) => {
            const delistTime =
                delistMap.get(symbol.symbol) || symbol.deliveryDate;
            return {
                ...
                onboardDateFormatted: new Date(
                    symbol.onboardDate,
                ).toISOString(),
                daysUntilDelisting: Math.ceil(
                    (delistTime - currentTime) / (24 * 60 * 60 * 1000),
                ),
                delistSource: delistMap.has(symbol.symbol)
                    ? "delist-schedule"
                    : "exchange-info",
            };
        });
 ...
 */
const getDelistingPerpetualContractsInfo = async (
    api_key,
    api_secret,
    daysAhead = 30,
) => {
    ...
    // 筛选即将下架的永续合约
    return getDelistingPerpetualContracts(
        latestInfo,
        delistSchedule,
        daysAhead,
    );
