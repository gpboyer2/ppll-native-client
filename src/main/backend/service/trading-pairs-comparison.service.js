...
const fetchCoinMFuturesTradingPairs = async () => {
    ...
    console.log(
        `币本位合约交易对数据已缓存，共 ${coinMFuturesPairs.length} 个交易对`,
    );
...
const getFuturesOnlyPairs = async (suffix = null) => {
    ...
    let futures_only_pairs = futuresPairs.filter(
        (pair) => !spotPairsSet.has(pair),
    );
...
const getSpotOnlyPairs = async (suffix = null) => {
    ...
    let spot_only_pairs = spotPairs.filter(
        (pair) => !futuresPairsSet.has(pair),
    );
...
const getComprehensiveReport = async (suffix = null) => {
    const [spotPairs, futuresPairs, futuresOnlyResult, spotOnlyResult] =
        await Promise.all([
            fetchSpotTradingPairs(),
            fetchFuturesTradingPairs(),
        ...
        assetAnalysis: {
            summary: {
                ...
                futures_only_assets_count:
                    assetAnalysis.futures_only_assets.length,
            },
