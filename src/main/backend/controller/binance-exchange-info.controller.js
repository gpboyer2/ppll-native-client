...
const getExchangeInfo = catchAsync(async (req, res) => {
    const { api_key, api_secret } = extractApiCredentials(req);

    try {
        // 检查是否需要更新
        if (await binanceExchangeInfoService.needsUpdate()) {
            const exchangeInfo =
                await binanceExchangeInfoService.fetchExchangeInfo(
                    api_key,
                    api_secret,
                );
        ...
        if (latestInfo.symbols && Array.isArray(latestInfo.symbols)) {
            latestInfo.symbols = filterUsdtPerpetualContracts(
                latestInfo.symbols,
            );
        }
...
const forceUpdate = catchAsync(async (req, res) => {
    ...
    try {
        ...
        if (exchangeInfo.symbols && Array.isArray(exchangeInfo.symbols)) {
            exchangeInfo.symbols = filterUsdtPerpetualContracts(
...
const getDelistingPerpetualContracts = catchAsync(async (req, res) => {
    ...
    try {
        const delistingContracts =
            await binanceExchangeInfoService.getDelistingPerpetualContractsInfo(
                api_key,
                api_secret,
                daysAhead,
            );
        ...
        return res.apiSuccess(
            ...
            delistingContracts.length > 0
                ? `发现${delistingContracts.length}个即将下架的永续合约`
                : "当前没有即将下架的永续合约",
        );
    } catch (err) {
...
const getDelistScheduleTest = catchAsync(async (req, res) => {
    ...
    try {
        const delistSchedule =
            await binanceExchangeInfoService.fetchDelistSchedule(
                api_key,
                api_secret,
            );
