...
const MANAGEMENT_TOKEN =
    process.env.MANAGEMENT_TOKEN || "your-secure-management-token";
...
function emergencyCleanup() {
    ...
    // 按最后访问时间排序，清理最老的IP记录
    const sortedIPs = Array.from(ipRequestCounts.entries()).sort(
        ([, a], [, b]) => {
            const aLastTime =
                a.requests[a.requests.length - 1]?.time || a.firstReqTime;
            const bLastTime =
                b.requests[b.requests.length - 1]?.time || b.firstReqTime;
    ...
    console.log(
        `[Rate Limit] 紧急清理了 ${cleanedCount} 个IP计数，当前内存IP数量: ${ipRequestCounts.size}`,
    );
}
    ...
    setInterval(
        async () => {
            try {
                ...
                if (cleanedCount > 0) {
                    console.log(
                        `[Rate Limit] 清理了 ${cleanedCount} 条过期封禁记录`,
                    );
                }
            } catch (error) {
...
function rateLimitMiddleware() {
    ...
    return async (req, res, next) => {
        ...
        if (isInternalServiceCall(req)) {
            console.log(
                `[Rate Limit] 内部服务调用，IP: ${ip}, 路径: ${req.path}`,
            );
            return next();
        }
        ...
        if (ipRequestCounts.size > RATE_LIMIT.MAX_MEMORY_IPS) {
            console.warn(
                `[Rate Limit] 内存中IP数量过多(${ipRequestCounts.size})，执行紧急清理`,
            );
            emergencyCleanup();
        }
    ...
    for (const [ip, data] of ipRequestCounts) {
        stats[ip] = {
            ...
            lastReqTime: new Date(
                data.requests[data.requests.length - 1]?.time ||
                    data.firstReqTime,
            ).toISOString(),
        };
    }
