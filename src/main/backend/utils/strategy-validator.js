...
function validateMinNotional(quantities, exchangeInfo, symbol, currentPrice) {
    ...
    for (const qty of quantities) {
        ...
        if (estimatedValue < minNotional) {
            ...
            const stepHint =
                stepSizeStr && stepSize > 0
                    ? `（按步长 ${stepSizeStr} 向上取整）`
                    : "";
            return {
                valid: false,
                message:
                    `根据币安官方要求，${symbol} 最小单笔交易金额为 ${minNotional.toFixed(2)} USDT。` +
...
function validatePriceDifference(priceDiff, exchangeInfo) {
    ...
    const isAligned =
        ...
        Math.abs(remainder - tickSize) < EPSILON;
