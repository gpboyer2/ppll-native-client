 ...
 */
function cleanOldFilesInDirectory(
    dirPath,
    dirName,
    ...
    try {
        ...
        UtilRecord.log(
            `开始清理 ${dirName} 目录中 ${cutoffDate.toISOString().split("T")[0]} 之前的文件（保留近${retentionDays}天）...`,
        );
            ...
            if (stats.isFile()) {
                ...
                if (stats.mtime < cutoffDate) {
                    try {
                        ...
                        totalSize += fileSize;
                        UtilRecord.log(
                            `已删除过期文件: ${file} (大小: ${(fileSize / 1024).toFixed(2)} KB)`,
                        );
                    } catch (deleteError) {
                        UtilRecord.log(
                            `删除文件 ${file} 时出错: ${deleteError.message}`,
                        );
                    }
                }
            }
        ...
        if (deletedCount > 0) {
            UtilRecord.log(
                `${dirName} 清理完成: 删除了 ${deletedCount} 个文件，释放了 ${(totalSize / 1024 / 1024).toFixed(2)} MB 空间`,
            );
        } else {
...
function startLogCleanupSchedule() {
    ...
    UtilRecord.log(
        `日志清理定时任务已启动，下次执行时间: ${new Date(Date.now() + timeUntilNext3AM).toLocaleString("zh-CN")}`,
    );
