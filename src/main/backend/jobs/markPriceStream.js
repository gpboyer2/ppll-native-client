...
const initMarkPriceStream = async () => {
    try {
        ...
        client.on("open", (data) => {
            UtilRecord.log(
                `✅ 标记价格流 WebSocket 连接已建立: ${data?.wsKey || "usdm"}`,
            );
        ...
        client.on("reconnecting", (data) => {
            UtilRecord.log(
                `标记价格流 WebSocket 正在重新连接: ${data?.wsKey || "usdm"}`,
            );
        });

        client.on("reconnected", (data) => {
            UtilRecord.log(
                `✅ 标记价格流 WebSocket 已重新连接: ${data?.wsKey || "usdm"}`,
            );
        });
        ...
        if (subscribeRetryCount < MAX_SUBSCRIBE_RETRY) {
            subscribeRetryCount++;
            UtilRecord.log(
                `将在 5 秒后重试初始化 (${subscribeRetryCount}/${MAX_SUBSCRIBE_RETRY})...`,
            );
            setTimeout(() => {
