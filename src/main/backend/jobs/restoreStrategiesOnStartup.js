...
async function getRunningStrategies() {
    try {
        ...
        const result = await gridService.getAllGridStrategys(
            {
                status: "RUNNING",
                [Op.or]: [{ paused: false }, { paused: null }],
            },
...
async function recoverSingleStrategy(strategy) {
    ...
    try {
        ...
        if (positionSide !== "LONG" && positionSide !== "SHORT") {
            UtilRecord.log(
                `⚠️  ${strategyInfo} 持仓方向无效(${s.position_side})，只支持LONG或SHORT，跳过恢复`,
            );
            recoveryStats.skipped++;
            return false;
        }
...
async function recoverStrategiesWithRateLimit(strategies) {
    ...
    UtilRecord.log(
        `📦 将分 ${totalBatches} 个批次恢复策略，每批次最多 ${batchSize} 个策略`,
    );
    ...
    for (let batchIndex = 0; batchIndex < totalBatches; batchIndex++) {
        ...
        UtilRecord.log(
            `📋 正在处理第 ${batchIndex + 1}/${totalBatches} 批次 (${batch.length} 个策略)...`,
        );

        // 并行处理当前批次的策略
        const batchPromises = batch.map((strategy) =>
            recoverSingleStrategy(strategy),
        );
        ...
        if (batchIndex < totalBatches - 1) {
            UtilRecord.log(
                `⏳ 等待 ${RECOVERY_RATE_LIMIT.delayBetweenBatches}ms 后处理下一批次...`,
            );
            await new Promise((resolve) =>
                setTimeout(resolve, RECOVERY_RATE_LIMIT.delayBetweenBatches),
            );
        }
    }
}
...
function printRecoveryStats() {
    ...
    if (recoveryStats.total > 0) {
        const successRate = (
            (recoveryStats.success / recoveryStats.total) *
            100
        ).toFixed(1);
...
async function startRecovery() {
    try {
        ...
        UtilRecord.log(
            `⚙️  恢复配置: 每秒最多${RECOVERY_RATE_LIMIT.maxPerSecond}个策略，批次间隔${RECOVERY_RATE_LIMIT.delayBetweenBatches}ms`,
        );
