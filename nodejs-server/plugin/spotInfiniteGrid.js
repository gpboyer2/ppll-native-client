...
function InfiniteGridSpot(options) {
  ...
  // 检查交易数量配置的有效性
  const hasGridTradeQuantity = options.grid_trade_quantity && options.grid_trade_quantity > 0;
  const hasSeparateQuantities = options.grid_long_open_quantity && options.grid_long_open_quantity > 0 &&
    options.grid_long_close_quantity && options.grid_long_close_quantity > 0;

  if (!hasGridTradeQuantity && !hasSeparateQuantities) {
    console.error(`❗️ 必须配置 'grid_trade_quantity' 或者同时配置 'grid_long_open_quantity' 和 'grid_long_close_quantity'，且值必须大于0`);
    return;
  }
  ...
  this.initAccountInfo = async () => {
    try {
      ...
      const [base_asset, quote_asset] = this.parseSymbol(this.config.trading_pair);
      const base_balance = account_info.balances.find(b => b.asset === base_asset);
      const quote_balance = account_info.balances.find(b => b.asset === quote_asset);
  ...
  this.getAverageCostPrice = async (symbol, days) => {
    const daysToCalculate = (days === null || days === undefined) ? this.config.avg_cost_price_days : days;
    ...
    try {
      ...
      for (const trade of trades) {
        ...
        if (trade && trade.isBuyer &&
          trade.quoteQty && !isNaN(parseFloat(String(trade.quoteQty))) &&
          trade.qty && !isNaN(parseFloat(String(trade.qty))) && parseFloat(String(trade.qty)) > 0) {
  ...
  this.resetTargetPrice = (execution_price) => {
    if (!execution_price || !this.config.grid_price_difference) {
      this.logger.warn(`重置期望价格失败，execution_price: ${execution_price}, grid_price_difference: ${this.config.grid_price_difference}`);
      return;
    }

    // 现货网格：低买高卖策略
    this.next_expected_rise_price = bigNumber(execution_price).plus(this.config.grid_price_difference).toNumber();

    // 应用防跌系数
    let coefficient = bigNumber(this.config.grid_price_difference)
      .times(bigNumber(this.current_base_asset_quantity).div(this.config.max_open_position_quantity || this.current_base_asset_quantity + 1))
      .times(this.config.fall_prevention_coefficient);
    coefficient = coefficient.isNaN() ? bigNumber(0) : coefficient;

    this.next_expected_fall_price = bigNumber(execution_price).minus(this.config.grid_price_difference).minus(coefficient).toNumber();
  };
  ...
  this.queryOrder = async (orderId, prePositionQty, orderQty, orderType) => {
    ...
    for (let i = 0; i <= MAX_RETRY; i++) {
      ...
      try {
        let res = await this.client.getOrder({ symbol: this.config.trading_pair, orderId: Number(orderId) });
    ...
    const isSuccess = Math.abs(this.current_base_asset_quantity - expectedQty) <= bigNumber(orderQty).times(0.001).toNumber();
    this.logger.log(`📊 持仓推断: 订单前=${prePositionQty}, 预期=${expectedQty}, 当前=${this.current_base_asset_quantity}, 推断${isSuccess ? '成功' : '失败'}`);
    if (typeof this.onWarn === 'function') {
  ...
  this.openOrders = async (quantity) => {
    ...
    this.total_open_position_entry_price = await this.getAverageCostPrice(this.config.trading_pair);
  ...
  this.closeOrders = async (quantity) => {
    ...
    const executionPrice = Number(orderDetail.cummulativeQuoteQty) / Number(orderDetail.executedQty);
    this.logs.push(orderDetail);
    if (this.position_open_history.length > 0) this.position_open_history.pop();
  ...
  this.getTotalAssetValue = (currentPrice) => {
    return bigNumber(this.current_base_asset_quantity).times(currentPrice).plus(this.current_quote_asset_balance).toNumber();
  };
  ...
  this.getGridProfit = (latestPrice) => {
    ...
    let buyValue = bigNumber(latestPrice).minus(this.config.grid_price_difference).times(buyQuantity);
  ...
  this.gridWebsocket = async ({ latestPrice }) => {
    if (!latestPrice) {
      this.logger.error(`InfiniteGridSpot gridWebsocket latestPrice error: `, latestPrice);
      return;
    }
    ...
    if (this.config.polling_interval) {
      ...
      setTimeout(() => this.throttle_enabled = false, this.config.polling_interval);
    }

    // 假设没有仓位时：
    //  - 初始化账户信息与仓位信息；
    if (!this.current_base_asset_quantity || !this.position_open_history?.length) {
      this.logger.warn(`⚠️ 当前已没有仓位信息，重新初始化账户信息与仓位信息用以同步最新数据`);
      this.logger.warn(`⚠️ this.current_base_asset_quantity`, this.current_base_asset_quantity);
      this.logger.warn(`⚠️ this.position_open_history`, this.position_open_history);
      await this.initAccountInfo().catch(() => { });
    }

    // 定期刷新账户信息，避免手动转入资金后无法及时更新余额的问题
    // 每100次轮询或超过5分钟未更新时强制刷新一次
    if (this.count % 100 === 0 || !this.last_account_info_update || (Date.now() - this.last_account_info_update) > 300000) {
      this.logger.log(`🔄 定期刷新账户信息以同步最新余额`);
      await this.initAccountInfo().catch(() => { });
    }

    this.logger.log(`----- ${dayjs().format('YYYY-MM-DD HH:mm:ss')} -----`);
    this.logger.log(`💰 现货网格, ID:${this.config.id} . 轮询第 ${this.count} 次`);
    this.count += 1;
    ...
    this.logger.log(`近${this.config.avg_cost_price_days}天平均持仓成本: ${this.total_open_position_entry_price}`);

    this.logger.log(`每次买入数量: ${buyQuantity}/${this.config.trading_pair}, 每次卖出数量: ${sellQuantity}/${this.config.trading_pair}, 网格价差: ${this.config.grid_price_difference} ${this.balance_info.quoteAsset}, 下次网格匹配利润预计为(扣除0.1%手续费): ${this.getGridProfit(latestPrice)} ${this.balance_info.quoteAsset}`);

    this.logger.log(`是否允许'顺势仅减仓策略': ${this.config.priority_close_on_trend}`);
    this.logger.log(`期望下次涨至: ${this.next_expected_rise_price}, 期望下次跌至: ${this.next_expected_fall_price}`);
    this.logger.log(`累计已成交 ${this.logs.length} 次`);
    this.logger.log(`当前持仓数量为 ${this.current_base_asset_quantity}/${this.config.trading_pair}, 限制最大持仓数量为 ${this.config.max_open_position_quantity}/${this.config.trading_pair}`);
    this.logger.log(`剩余未匹配平仓的订单: `, this.position_open_history);

    // 如果没有期望价格，初始化
    if ((!this.next_expected_rise_price || !this.next_expected_fall_price) && this.logs.length) {
    ...
    if (
      ...
      && (this.config.max_open_position_quantity ? this.current_base_asset_quantity < this.config.max_open_position_quantity : true)
    ) {
      if (this.current_quote_asset_balance < bigNumber(latestPrice).times(buyQuantity).toNumber()) {
      ...
      if (
        ...
        this.logger.log(`🔄 启用顺势仅减仓策略：当前实际仓位数量为 ${this.current_base_asset_quantity} / ${this.config.trading_pair}， 足够平仓，且当前仍处于上涨趋势，因此跳过创建新仓位`);
      } else {
        this.logger.log(`😎 缓存中没有仓位且没有超过最大持仓数量限制, 增加一个新的仓位`);
    ...
    if (
      ...
      this.current_base_asset_quantity >= (this.config.min_open_position_quantity || 0)
    ) {
      this.logger.log(`⬆️ 价格上涨，执行平仓操作. 匹配上一个网格的价格为：`, lastPosition?.cummulativeQuoteQty);
    ...
    // 如果基础资产少于最小持仓要求，立即买入
    if (this.config.min_open_position_quantity &&
      this.current_base_asset_quantity < this.config.min_open_position_quantity
    ) {
      if (this.current_quote_asset_balance < bigNumber(latestPrice).times(buyQuantity).toNumber()) {
        await this.updateExecutionStatus(execution_status.INSUFFICIENT_BALANCE);
        this.logger.log(`余额不足，无法执行买入操作`);
  ...
  this._initOrders = async () => {
    ...
    // 初始化时获取准确的平均持仓成本
    this.total_open_position_entry_price = await this.getAverageCostPrice(this.config.trading_pair);
    this.logger.log(`📈 初始平均持仓成本: ${this.total_open_position_entry_price}`);

    let { min_open_position_quantity } = this.config;
    let buyQuantity = this.getLongOpenQuantity();

    // 如果基础资产少于最小持仓要求，补仓
    if (min_open_position_quantity && this.current_base_asset_quantity < min_open_position_quantity) {
      let quantity = bigNumber(min_open_position_quantity).minus(this.current_base_asset_quantity).plus(buyQuantity).toNumber();
      ...
      if (this.latestPrice && this.latestPrice > 0) {
        ...
        if (this.current_quote_asset_balance >= requiredQuote) {
          await this.openOrders(quantity).catch((error) => this.logger.error('补仓失败:', error));
        } else {
          this.logger.log(`计价资产不足，需要 ${requiredQuote}，当前仅有 ${this.current_quote_asset_balance}`);
        }
      } else {
