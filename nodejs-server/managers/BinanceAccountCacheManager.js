...
const set = (api_key, marketType, accountData) => {
  ...
  UtilRecord.debug(`[缓存管理器] 更新缓存 - api_key: ${api_key.substring(0, 8)}..., market: ${marketType}`);
};
...
const updateFromWebSocket = (api_key, marketType, eventData) => {
  try {
    ...
    if (!user_cache || !user_cache[marketType]) {
      // 缓存不存在，无法增量更新
      UtilRecord.debug(`[缓存管理器] 缓存不存在，跳过 WebSocket 增量更新 - api_key: ${api_key.substring(0, 8)}..., market: ${marketType}`);
      return false;
    }
    ...
    // 更新余额信息
    if (updateData.updatedBalances && Array.isArray(updateData.updatedBalances)) {
      // 确保 balances 数组存在
    ...
    // 更新持仓信息（合约市场）
    if (updateData.updatedPositions && Array.isArray(updateData.updatedPositions)) {
      // 确保 positions 数组存在
      if (!cached_data.positions || !Array.isArray(cached_data.positions)) {
      ...
      for (const updatedPosition of updateData.updatedPositions) {
        ...
        if (existing) {
          ...
          existing.unRealizedProfit = updatedPosition.unRealizedProfit;
    ...
    if (updated) {
      ...
      UtilRecord.debug(`[缓存管理器] WebSocket 增量更新成功 - api_key: ${api_key.substring(0, 8)}..., market: ${marketType}`);
    }

    return updated;
  } catch (error) {
    // 缓存更新失败不应影响原有事件处理逻辑
    UtilRecord.log(`[缓存管理器] WebSocket 增量更新失败 - ${error.message}`);
    return false;
  }
};
...
const clear = (api_key) => {
  ...
  UtilRecord.debug(`[缓存管理器] 清除缓存 - api_key: ${api_key.substring(0, 8)}...`);
};
...
const clearMarket = (api_key, marketType) => {
  ...
  if (user_cache) {
    ...
    UtilRecord.debug(`[缓存管理器] 清除市场缓存 - api_key: ${api_key.substring(0, 8)}..., market: ${marketType}`);
  }
};
