...
const logger = CrawlerLog.createLogger({ source: 'gate', consoleOutput: false });
...
const datum = {
    ...
    setFileInfo: function (data) {
        return false; // 暂时禁用写入功能, 因为可以直接读 global 的数据
        try {
            // 确保目录存在
...
function calculateSmartDelay(pageNumber, dataLength = 50) {
    ...
    if (requestStats.consecutiveErrors >= config.consecutiveErrorThreshold) {
        delay *= Math.pow(config.errorBackoffMultiplier, requestStats.consecutiveErrors - config.consecutiveErrorThreshold + 1);
    }
...
function calculateBackoffDelay() {
    ...
    // 指数退避算法
    const exponentialDelay = baseBackoff * Math.pow(
 ...
 */
async function fetchGateCoinList(tab = 'trade', sort = 'dimension_24h', order = 'desc', enableProgressiveUpdate = false) {
    ...
    // 分页获取数据
    for (let page = 1; page <= 1000; page++) { // 设置最大页数限制防止无限循环
        try {
            ...
            if (data && data.code === 0 && data.data && data.data.list) {
                ...
                if (pageData.length === 0) {
                    ...
                    logger.log(`[Gate Coin Cache] ${tab} 数据获取完成，总共 ${result.length} 条记录`);
                    return result;
                }
                ...
                pageData.forEach(coin => {
                    if (coin.coin_short_name) {
                        global.GATE_COIN_CACHE.coinMap[coin.coin_short_name] = coin;
                    }
                });

                logger.log(`[Gate Coin Cache] 获取 ${tab} 第${page}页，本页 ${pageData.length} 条，累计 ${Object.keys(global.GATE_COIN_CACHE.coinMap).length} 个（${responseTime}ms）`);

                // 每页都同步更新内存缓存
                const currentData = Object.values(global.GATE_COIN_CACHE.coinMap);
                ...
                logger.log(`[Gate Coin Cache] ${tab} 第${page}页已更新到内存缓存 (${currentData.length}条)`);
            ...
            if (pageError.response && pageError.response.status === 429) {
                ...
                logger.warn(`[Gate Coin Cache] ${tab} 第${page}页遇到限流，等待 ${Math.round(backoffDelay / 1000)} 秒后重试...`);
                await new Promise(resolve => setTimeout(resolve, backoffDelay));
                page--; // 重试当前页，所以减一
            } else if (pageError.response && pageError.response.status === 403) {
                // 遇到403错误，尝试刷新cookies后重试
                logger.warn(`[Gate Coin Cache] ${tab} 第${page}页遇到403错误，尝试刷新cookies...`);
                try {
                    const freshCookies = await getValidCookies(true);
                    // 更新cookies变量
                    cookies = freshCookies;
                    // 403错误后也要等待一段时间
                    await new Promise(resolve => setTimeout(resolve, RATE_LIMIT_CONFIG.updateStrategy?.bufferBetweenTypes));
                    page--; // 重试当前页，所以减一
                } catch (cookieError) {
                ...
                if (requestStats.consecutiveErrors < 5) {
                    ...
                    logger.warn(`[Gate Coin Cache] ${tab} 第${page}页请求失败，等待 ${Math.round(errorDelay / 1000)} 秒后重试...`);
                    await new Promise(resolve => setTimeout(resolve, errorDelay));
                    page--; // 重试当前页，所以减一
                } else {
                    logger.error(`[Gate Coin Cache] ${tab} 连续失败次数过多，停止请求`);
                    const result = Object.values(global.GATE_COIN_CACHE.coinMap);
                    logger.log(`[Gate Coin Cache] ${tab} 数据获取完成，总共 ${result.length} 条记录`);
                    return result;
                }
...
async function updateGateCoinCache() {
    try {
        ...
        const gainersData = await fetchGateCoinList('crypto-gainers', 'dimension_24h', 'desc');

        // 立即更新涨幅榜数据到全局缓存
        global.GATE_COIN_CACHE.gainers = gainersData;
        global.GATE_COIN_CACHE.lastUpdate = new Date().toISOString();
        logger.log(`[Gate Coin Cache] 涨幅榜数据已更新 (${gainersData.length} 条)`);

        // 在不同数据类型之间添加缓冲时间
        await new Promise(resolve => setTimeout(resolve, RATE_LIMIT_CONFIG.updateStrategy?.bufferBetweenTypes));

        logger.log('[Gate Coin Cache] 开始获取跌幅榜数据...');
        const losersData = await fetchGateCoinList('crypto-losers', 'dimension_24h', 'asc');

        // 立即更新跌幅榜数据到全局缓存
        global.GATE_COIN_CACHE.losers = losersData;
        global.GATE_COIN_CACHE.lastUpdate = new Date().toISOString();
        logger.log(`[Gate Coin Cache] 跌幅榜数据已更新 (${losersData.length} 条)`);
        ...
        logger.log(`[Gate Coin Cache] 缓存更新完成，用时 ${Math.round(duration / 1000)}s`);
        logger.log(`[Gate Coin Cache] 涨幅榜: ${gainersData.length} 条，跌幅榜: ${losersData.length} 条，全部: ${allData.length} 条`);
...
function startGateCoinCacheJob() {
    ...
    logger.log(`[Gate Coin Cache] 定时任务已启动，每 ${interval / 60000} 分钟更新一次`);
}
