...
function smartAdjustQuantity(exchangeInfo, symbol, quantity, options = {}) {
  ...
  if (!silent) {
    ...
    const precision = lotSizeFilter ? getDecimalPlaces(lotSizeFilter.stepSize) : 8;
  ...
  // 根据过滤器调整数量
  const adjustedQuantity = adjustQuantityByFilters(symbol, quantity, filters, silent);
...
function smartAdjustPrice(exchangeInfo, symbol, price, options = {}) {
  ...
  if (!priceFilter) {
    if (!silent) {
      UtilRecord.log(`[价格精度] ${symbol} 缺少PRICE_FILTER过滤器，使用默认精度`);
    }
  ...
  if (adjustedPrice.isLessThan(minPrice)) {
    ...
    if (!silent) {
      UtilRecord.log(`[价格调整] ${symbol} 价格低于最小值 - 原始: ${price}, 最小限制: ${minPrice}, 调整后: ${adjustedPrice.toString()}`);
    }
  }
  ...
  if (adjustedPrice.isGreaterThan(maxPrice)) {
    ...
    if (!silent) {
      UtilRecord.log(`[价格调整] ${symbol} 价格超过最大值 - 原始: ${price}, 最大限制: ${maxPrice}, 调整后: ${adjustedPrice.toString()}`);
    }
  }
  ...
  if (tickSize && tickSize !== '0') {
    ...
    if (!silent && originalPrice !== result) {
      UtilRecord.log(`[价格精度] ${symbol} 价格精度调整 - 原始: ${originalPrice}, 调整后: ${result}, 精度要求: ${precision}位小数`);
    }
