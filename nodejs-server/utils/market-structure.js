...
const findSwingHighList = (klineList, atr) => {
  ...
  for (let i = confirmBars; i < klineList.length - confirmBars; i++) {
    ...
    // 验证回调深度（至少0.5个ATR）
    if (hasRetrace && retraceDepth >= atr * CONFIG.SWING.MIN_SWING_SIZE_ATR) {
      swingHighList.push({
        price: current.high,
        index: i,
...
const findSwingLowList = (klineList, atr) => {
  ...
  for (let i = confirmBars; i < klineList.length - confirmBars; i++) {
    ...
    // 验证反弹高度（至少0.5个ATR）
    if (hasBounce && bounceHeight >= atr * CONFIG.SWING.MIN_SWING_SIZE_ATR) {
      swingLowList.push({
        price: current.low,
        index: i,
        openTime: current.openTime,
...
const findConsolidationPlatformList = (klineList, avgVolume) => {
  ...
  for (let start = 0; start < klineList.length - minBars; start++) {
    ...
    for (let end = start + 1; end < Math.min(start + maxBars, klineList.length); end++) {
...
class KeyLevelMemory {
  constructor(symbol) {
    this.symbol = symbol;
    this.levelList = [];
    this.cacheFile = path.join(CONFIG.CACHE_DIR, `${symbol}-key-levels.json`);
    this.load();
  }
  ...
  load() {
    try {
      if (fs.existsSync(this.cacheFile)) {
        ...
        console.log(`[记忆库] 加载 ${this.symbol} 历史关键位: ${this.levelList.length} 个`);
      }
    } catch (error) {
  ...
  addLevel(level) {
    ...
    const existingIndex = this.levelList.findIndex(existing => {
      const deviation = Math.abs(existing.price - level.price) / level.price;
    ...
    if (existingIndex >= 0) {
      ...
      existing.strength = Math.max(existing.strength, level.strength || 1);
      // 静默更新，不输出日志
    } else {
      // 添加新的关键位
      this.levelList.push({
  ...
  applyTimeDecay() {
    ...
    this.levelList = this.levelList.filter(level => {
      ...
      const decay = daysSinceLastTest * CONFIG.EFFICACY.TIME_DECAY_RATE * level.efficacy;
  ...
  getSupportLevelList(currentPrice) {
    return this.getEffectiveLevelList(currentPrice)
...
const analyzeMarketStructure = (options) => {
  ...
  // 计算平均成交量
  const avgVolume = klineList.reduce((sum, k) => sum + k.volume, 0) / klineList.length;
  ...
  if (dailyKlineList && dailyKlineList.length >= 20) {
    const dailyAvgVolume = dailyKlineList.reduce((sum, k) => sum + k.volume, 0) / dailyKlineList.length;
    ...
    const dailyPlatformList = findConsolidationPlatformList(dailyKlineList, dailyAvgVolume);
