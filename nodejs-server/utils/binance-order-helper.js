 ...
 */
async function placeOrderByValue(orderParams, apiKey, apiSecret, maxRetries = 3) {
  const { symbol, side, positionSide, notionalValue, currentPrice } = orderParams;

  console.log(`开始按价值下单: ${symbol}, 价值: ${notionalValue} USDT, 价格: ${currentPrice}`);
  ...
  for (let attempt = 1; attempt <= maxRetries; attempt++) {
    try {
      console.log(`第 ${attempt} 次尝试下单: ${symbol}, 数量: ${quantity}`);
      ...
      lastError = error;
      console.warn(`❌ ${symbol} 第 ${attempt} 次下单失败: ${error.message || error}`);
      ...
      // 如果是余额不足等其他错误，直接中断
      if (typeof error === 'string' && (error.includes('-2019') || error.includes('-1013'))) {
        break;
      }
    }
  }
 ...
 */
async function createPositionOrders(symbol, longValue, shortValue, apiKey, apiSecret) {
