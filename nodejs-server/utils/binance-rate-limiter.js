...
class BinanceRateLimiter {
  ...
  clearCacheByApiKey(apiKey) {
    ...
    if (cleared > 0) {
      UtilRecord.debug(`[RateLimiter] 清除了 ${cleared} 个缓存项 (apiKey: ${keyPrefix}...)`);
    }
  }
  ...
  async execute(apiCall, options = {}) {
    ...
    if (useCache) {
      ...
      if (cached) {
        UtilRecord.debug(`[RateLimiter] 缓存命中: ${method} (${apiKey.substring(0, 8)}...)`);
        return cached;
      }
    }
    ...
    while (this.requestQueue.length > 0) {
      ...
      try {
        ...
        if (timeSinceLastRequest < this.minDelayMs) {
          ...
          UtilRecord.debug(`[RateLimiter] 等待 ${delay}ms 后执行下一个请求...`);
          await new Promise(resolve => setTimeout(resolve, delay));
        }

        // 执行API调用
        task.attempt++;
        UtilRecord.debug(`[RateLimiter] 执行API请求: ${task.options.method} (尝试 ${task.attempt}/${task.retries})`);
  ...
  isRateLimitError(error) {
    ...
    // 检查响应头
    if (error.headers) {
      const connection = error.headers.connection || error.headers.Connection;
      const contentLength = error.headers['content-length'] || error.headers['Content-Length'];
