...
const { calculateMA, calculateBollingerBands } = require('./technical-indicator.js');
const { getHistoricalEfficacy, analyzeMarketStructure } = require('./market-structure.js');
...
const identifyPinBar = (kline) => {
  ...
  const effectiveBodyHeight = bodyHeight > 0 ? bodyHeight : (kline.high - kline.low) * 0.1;
...
const getTouchType = (bucketCenter, kline) => {
  ...
  if ((bucketCenter > bodyHigh && bucketCenter <= Math.min(nearWickUpper, kline.high)) ||
    (bucketCenter < bodyLow && bucketCenter >= Math.max(nearWickLower, kline.low))) {
    return 'near_wick';
  }
...
const findSwingPoints = (klineList) => {
  ...
  if (!klineList || klineList.length === 0) {
    return { supportCandidateList, resistanceCandidateList, priceLevelMap: [] };
  }
  ...
  const avgTouchWeight = peakList.length > 0
    ? peakList.reduce((sum, p) => sum + p.touchWeight, 0) / peakList.length
    : 0;
  ...
  for (let i = swingWindow; i < klineList.length - swingWindow; i++) {
    ...
    if (isSwingLow && effectiveLow < currentPrice) {
      // 检查是否已有相近的候选点
      const existingNear = supportCandidateList.find(c =>
        Math.abs(c.price - effectiveLow) / effectiveLow < 0.005
      );
      if (!existingNear) {
    ...
    if (isSwingHigh && effectiveHigh > currentPrice) {
      // 检查是否已有相近的候选点
      const existingNear = resistanceCandidateList.find(c =>
...
const findVolumeFortress = (klineList) => {
  ...
  for (const kline of klineList) {
    ...
    for (let i = 0; i < bucketList.length; i++) {
      ...
      // 影线部分覆盖的格子（排除实体部分）
      const inUpperWick = bucket.upperBound > bodyHigh && bucket.lowerBound < kline.high;
      const inLowerWick = bucket.upperBound > kline.low && bucket.lowerBound < bodyLow;
  ...
  // 按成交量排序，找出最高的3个格子
  const sortedBucketList = [...bucketList].sort((a, b) => b.volume - a.volume);
...
const findDailySwingPoints = (dailyKlineList) => {
  ...
  // 使用共识水平位算法
  const { supportCandidateList, resistanceCandidateList } = findSwingPoints(dailyKlineList);
 ...
 */
const scoreCandidate = (candidate, klineList, ma20, dailyLevelPriceList, symbol, priceRange, currentPrice) => {
  let score = 0;
  ...
  if (candidate.touchWeight && candidate.avgTouchWeight) {
    const threshold = candidate.avgTouchWeight * CONFIG.THRESHOLD.HIGH_TOUCH_WEIGHT_MULTIPLIER;
  ...
  if (candidate.source === 'consolidation_level' && candidate.touchCount) {
    ...
    const touchBonus = Math.min(candidate.touchCount - CONFIG.PRICE_LEVEL.MIN_TOUCH_COUNT, 10);
    score += touchBonus;
  ...
  if (priceRange && priceRange.minPrice && priceRange.maxPrice) {
    ...
    if (distanceFromMin <= extremeThreshold || distanceFromMax <= extremeThreshold) {
  ...
  if (currentPrice && priceRange && priceRange.minPrice && priceRange.maxPrice) {
    const range = priceRange.maxPrice - priceRange.minPrice;
    ...
    const maxDistanceRatio = 0.20;  // 最大距离20%

    if (distanceRatio >= minDistanceRatio && distanceRatio <= maxDistanceRatio) {
      // 线性递减：距离2%时加满分，距离20%时加0分
      const effectiveRange = maxDistanceRatio - minDistanceRatio;
      const positionInRange = (distanceRatio - minDistanceRatio) / effectiveRange;
      const proximityBonus = Math.round(CONFIG.SCORE.PROXIMITY_BONUS * (1 - positionInRange));
  ...
  // 7. 历史关键位效力加分（压倒性权重）
  if (symbol) {
    const historicalEfficacy = getHistoricalEfficacy(symbol, candidate.price);
    if (historicalEfficacy > 0) {
      // 效力分数按比例转换为加分（最高80分）
      const efficacyBonus = Math.min(
        Math.round(historicalEfficacy * CONFIG.SCORE.HISTORICAL_EFFICACY / 100),
...
const identifySupportResistance = (options) => {
  ...
  if (symbol && atr) {
    try {
      ...
      if (structureResult.success) {
        console.log(`[结构分析] ${symbol} 历史关键位: 支撑${structureResult.historicalLevelList.support.length}个, 阻力${structureResult.historicalLevelList.resistance.length}个`);
      }
    } catch (error) {
  ...
  // 规则A：波段极点
  const { supportCandidateList: swingSupportList, resistanceCandidateList: swingResistanceList, priceRange } =
    findSwingPoints(klineList);

  // 规则B：成交量堡垒
  const { volumeCenterPrice, volumeBucketList } = findVolumeFortress(klineList);

  // 规则C：MA20均线
  const ma20 = calculateMA(klineList, CONFIG.MA_PERIOD);

  // 规则D：日线级别波段极点
  const { dailySupportList, dailyResistanceList } = findDailySwingPoints(dailyKlineList);
  ...
  const scoredSupportList = swingSupportList
    ...
    .map(c => scoreCandidate(c, klineList, ma20, dailySupportList, symbol, priceRange, currentPrice));

  // 为阻力候选点计分
  const scoredResistanceList = swingResistanceList
    .filter(c => c.price > currentPrice) // 阻力位必须高于当前价格
    .map(c => scoreCandidate(c, klineList, ma20, dailyResistanceList, symbol, priceRange, currentPrice));

  // 选出获胜者（优先选择波段极点配对）
  // 策略：优先选择价格最极端的波段极点（最低支撑 + 最高阻力）
  const findBestPair = () => {
    // 按得分排序
    const sortedSupport = [...scoredSupportList].sort((a, b) => b.score - a.score);
    const sortedResistance = [...scoredResistanceList].sort((a, b) => b.score - a.score);
    ...
    for (const s of swingSupport) {
      for (const r of sortedResistance) {
        const spread = (r.price - s.price) / currentPrice;
        if (spread >= CONFIG.THRESHOLD.MIN_SPREAD_RATIO) {
          return { support: s, resistance: r, source: 'swing_support' };
        }
      }
    }
    ...
    for (const s of sortedSupport) {
      for (const r of swingResistance) {
        const spread = (r.price - s.price) / currentPrice;
        if (spread >= CONFIG.THRESHOLD.MIN_SPREAD_RATIO) {
          return { support: s, resistance: r, source: 'swing_resistance' };
        }
      }
    }
  ...
  let resistanceSource = resistanceWinner ? 'algorithm' : 'bollinger_fallback';
...
const formatResult = (result) => {
  if (result.success) {
    ...
    if (result.meta.supportDetail) {
      lines.push(`  支撑位得分: ${result.meta.supportDetail.score} (来源: ${result.meta.supportDetail.source})`);
    }
    if (result.meta.resistanceDetail) {
      lines.push(`  阻力位得分: ${result.meta.resistanceDetail.score} (来源: ${result.meta.resistanceDetail.source})`);
    }
    ...
    if (result.supportLevelList && result.supportLevelList.length > 0) {
      ...
      result.supportLevelList.forEach(s => {
        lines.push(`    S${s.level}: ${s.price.toFixed(2)} (${s.source}, 得分${s.score.toFixed(0)})`);
      });
    }
    if (result.resistanceLevelList && result.resistanceLevelList.length > 0) {
      lines.push(`  多级阻力位:`);
      result.resistanceLevelList.forEach(r => {
        lines.push(`    R${r.level}: ${r.price.toFixed(2)} (${r.source}, 得分${r.score.toFixed(0)})`);
      });
    }
