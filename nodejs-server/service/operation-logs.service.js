...
function parseUa(ua = '') {
  ...
  else if (text.includes('iphone') || text.includes('ipad') || text.includes('ios')) os = 'iOS';
...
function toViewObject(data) {
  ...
  if (typeof status === 'undefined') {
    if (typeof extra.status !== 'undefined') status = String(extra.status) === '0' ? 0 : 1;
...
function buildWhere(filter = {}) {
  ...
  if (status !== undefined && status !== null && status !== '') where.status = Number(status) === 1 ? 1 : 0;
  if (start || end) {
