...
const RATE_LIMIT_CONFIG = {
  ...
  // 封禁时长（毫秒）- 永久封禁设为100年
  BAN_TIME_MS: parseInt(process.env.RATE_LIMIT_BAN_TIME_MS) || 100 * 365 * 24 * 60 * 60 * 1000,

  // 清理任务间隔（毫秒）
  CLEANUP_INTERVAL_MS: parseInt(process.env.RATE_LIMIT_CLEANUP_INTERVAL_MS) || 60 * 1000,
...
const TRUSTED_IPS = (process.env.TRUSTED_IPS || '').split(',').filter(ip => ip.trim());

// 管理操作令牌（用于内部服务调用）
const MANAGEMENT_TOKEN = process.env.MANAGEMENT_TOKEN || 'your-secure-management-token-' + Math.random().toString(36);
