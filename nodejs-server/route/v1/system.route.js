/**
 * 系统路由模块
 * 定义系统相关的API路由，提供系统信息查询功能
 */
const express = require('express');
const router = express.Router();
const systemController = require('../../controller/system.controller.js');


/**
 * 获取系统健康状态
 * GET /v1/system/health
 */
router.get('/health', systemController.getHealth);


/**
 * 获取本机 IPv4 地址列表
 * GET /v1/system/ipv4-list
 */
router.get('/ipv4-list', systemController.getIPv4List);


/**
 * 获取 Git 信息
 * GET /v1/system/git-info
 */
router.get('/git-info', systemController.getGitInfo);


// ==================== 数据库管理路由 ====================

/**
 * 获取数据库概览信息
 * GET /v1/system/database/info
 */
router.get('/database/info', systemController.getDatabaseInfo);


/**
 * 获取表列表
 * POST /v1/system/database/tables  body: { currentPage, pageSize, keyword }
 */
router.post('/database/tables', systemController.getTableList);


/**
 * 获取表结构详情
 * POST /v1/system/database/table-detail  body: { tableName }
 */
router.post('/database/table-detail', systemController.getTableDetail);


/**
 * 获取表数据
 * POST /v1/system/database/table-data  body: { tableName, currentPage, pageSize, sortBy, sortOrder }
 */
router.post('/database/table-data', systemController.getTableData);


/**
 * 创建数据
 * POST /v1/system/database/data-create  body: { tableName, data: [] }
 */
router.post('/database/data-create', systemController.createData);


/**
 * 更新数据
 * PUT /v1/system/database/data-update  body: { tableName, data: [] }
 */
router.put('/database/data-update', systemController.updateData);


/**
 * 删除数据
 * DELETE /v1/system/database/data-delete  body: { tableName, data: [] }
 */
router.delete('/database/data-delete', systemController.deleteData);


/**
 * 执行 SQL 查询
 * POST /v1/system/database/query  body: { sql, queryParams: [] }
 */
router.post('/database/query', systemController.executeQuery);


/**
 * 创建表
 * POST /v1/system/database/table-create  body: { tableName, columns: [] }
 */
router.post('/database/table-create', systemController.createTable);


/**
 * 删除表
 * DELETE /v1/system/database/table-delete  body: { data: [] }
 */
router.delete('/database/table-delete', systemController.deleteTable);


/**
 * 添加列
 * POST /v1/system/database/column-create  body: { tableName, columnName, type, nullable, defaultValue }
 */
router.post('/database/column-create', systemController.createColumn);


/**
 * 删除列
 * DELETE /v1/system/database/column-delete  body: { tableName, columnName }
 */
router.delete('/database/column-delete', systemController.deleteColumn);


module.exports = router;

/**
 * @swagger
 * tags:
 *   name: System
 *   description: 系统信息
 */

/**
 * @openapi
 * /v1/system/ipv4-list:
 *  get:
 *     tags: [System]
 *     summary: 获取本机 IPv4 地址列表
 *     description: 获取本机所有网络接口的 IPv4 地址（不包括回环地址）
 *     responses:
 *       200:
 *         description: 成功获取 IPv4 地址列表
 *         content:
 *           application/json:
 *             schema:
 *               type: object
 *               properties:
 *                 status:
 *                   type: string
 *                   example: "success"
 *                 code:
 *                   type: integer
 *                   example: 200
 *                 data:
 *                   type: array
 *                   items:
 *                     type: string
 *                   example: ["192.168.1.100", "10.0.0.5"]
 *                   description: IPv4 地址数组
 *       500:
 *         description: 服务器内部错误
 *         content:
 *           application/json:
 *             schema:
 *               type: object
 *               properties:
 *                 status:
 *                   type: string
 *                   example: "error"
 *                 code:
 *                   type: integer
 *                   example: 500
 *                 message:
 *                   type: string
 *                   example: "获取 IPv4 地址列表失败"
 */
