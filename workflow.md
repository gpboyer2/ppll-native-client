...
### 子代理1：校验完成情况和质量
- 根据用户的需求批判式的理解上一级分派的任务，不能误解用户的需求，然后找出 必须优化 和 必须修改 的地方
- 如果有 必须优化 和 必须修改 的地方，分发给子代理2
- 如果没有 必须优化 和 必须修改 的地方，通知主代理验收和校验本次任务完成情况
...
- 若验证通过：通知主代理重新启动子代理1重新校验（"重新"的意义在于确保所有修改都符合要求，避免遗漏，可以查漏补缺）

## 执行流程

1. **初始启动**：主代理（你）启动子代理1，传入用户需求
2. **主动监控**：主代理需要启动工具，脚本或者子代理，目的是每 60 秒使用 TaskOutput 检查子代理状态和输出，每次检查时判断是否存在停滞、死循环、错误卡住、离题等异常迹象，发现异常立即终止并重启，告知子代理避免重复问题。同时如果发现卡顿超过 5 分钟则强制终止并重启
3. **循环执行**：各子代理按工作流图执行，直到满足完成条件
4. **最终验收**：子代理1确认无需优化/修改后，通知主代理进行最终总结、约束复盘和整体校验
5. **正式结束**：主代理确认全局复盘与校验全部通过后，需求正式结束，生成一个中文的 git commit 消息(禁止提交)，终止所有子代理
