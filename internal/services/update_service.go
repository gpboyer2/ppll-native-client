...
func jsonNewDecoder(r any) jsonDecoder {
	return &stdJSONDecoder{r: r.(interface{ Read([]byte) (int, error) })}
}
...
func pickByPlatform(arr []map[string]any) int {
	...
	for i, it := range arr {
		...
		switch os {
		case "darwin":
			if strings.HasSuffix(u, ".dmg") || strings.HasSuffix(u, ".pkg") {
				return i
			}
		case "windows":
			if strings.HasSuffix(u, ".exe") || strings.HasSuffix(u, ".msi") {
				return i
			}
		default:
			if strings.HasSuffix(u, ".appimage") || strings.HasSuffix(u, ".tar.gz") || strings.HasSuffix(u, ".deb") || strings.HasSuffix(u, ".rpm") {
				return i
			}
		}
	}
...
func (s *UpdateService) normalizeUpdateInfo(m map[string]any) UpdateInfo {
	...
	if size == 0 {
		size = int64From(m["bytes"])
	}
	if size == 0 {
		size = int64From(m["filesize"])
	}
	notes := strFrom(m["notes"])
	if notes == "" {
		notes = strFrom(m["changelog"])
	}
	if notes == "" {
		notes = strFrom(m["desc"])
	}
